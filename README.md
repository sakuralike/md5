...
> 当前阶段：WP2/WP3 主要业务与跨浏览器预检已形成自动化基线；WP4 已完成供应链、密钥轮换、恢复、监控、证据与自动发布合同，用户等级系统已完成并部署到 Staging。目标服务器已完成双 API、三 Worker、单 Scheduler、监控和 78 秒短时回归；固定 4 小时稳定性验证不再是必需门禁，真实 MySQL/Redis HA、真实通知、生产恢复/密钥演练和 Windows 10/11 实机兼容性仍需独立验收。项目只使用合成测试数据，禁止提交真实密码、访问令牌或生产密钥。
