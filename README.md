...
> 当前阶段：M4 第 4 次迭代已完成状态驱动的积分补偿、信誉校正、人工首次验证结算和管理端调整时间线，候选人工审核、举报/申诉、用户积分/信誉中心及 M3 桌面发布闭环保持完成。当前工程估算为：功能代码约 86%、MVP 验收约 71%、生产上线准备度约 43%；该估算不替代目标环境验收。项目只使用合成测试数据，禁止提交真实密码、访问令牌或生产密钥。
