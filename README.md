...
> 当前阶段：M4 第 3 次迭代已完成积分投影、不可变信誉事件和用户积分/信誉/反馈中心，候选人工审核、举报/申诉和 M3 桌面发布闭环保持完成。当前工程估算为：功能代码约 84%、MVP 验收约 69%、生产上线准备度约 42%；该估算不替代目标环境验收。项目只使用合成测试数据，禁止提交真实密码、访问令牌或生产密钥。
