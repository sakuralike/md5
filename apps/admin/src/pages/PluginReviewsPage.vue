...
const sourceFiles = ref<Array<{ path: string; content: string; truncated: boolean }>>([]);
...
<template>
  <section class="space-y-6 pt-16 lg:pt-0">
    ...
    <Card v-if="selected"><CardHeader><CardTitle>版本审核详情操作</CardTitle></CardHeader><CardContent class="space-y-3"><div class="flex flex-wrap items-end gap-2"><div class="space-y-2"><Label for="remediation-days-detail">整改时间（天）</Label><Input id="remediation-days-detail" v-model.number="remediationDays" type="number" min="1" max="30" class="h-10 w-24" /></div><Button v-if="selected && ['auto_review_failed','review_queued'].includes(selected.status)" size="sm" variant="outline" :disabled="busy" @click="rerun">重新执行自动审核</Button><Button v-if="selected" size="sm" variant="destructive" :disabled="busy" @click="removeSelected">删除插件版本</Button><Button v-if="selected" size="sm" variant="outline" @click="router.push(`/plugin-reviews/${selected.version_id}/source`)">打开源码审查</Button></div></CardContent></Card>
    ...
    <Card v-if="selected"><CardHeader><CardTitle>提交源码</CardTitle><CardDescription>只显示插件包内显式提供的文本源码，二进制不在此处回显。</CardDescription></CardHeader><CardContent class="space-y-3"><article v-for="file in sourceFiles" :key="file.path" class="space-y-2 rounded-md border p-3"><code class="block text-xs">{{ file.path }}</code><pre class="max-h-80 overflow-auto whitespace-pre-wrap rounded bg-muted p-3 text-xs">{{ file.content }}</pre><p v-if="file.truncated" class="text-xs text-muted-foreground">文件已截断显示。</p></article><p v-if="sourceFiles.length === 0" class="text-sm text-muted-foreground">该版本未随包提供可审源码。</p></CardContent></Card>
