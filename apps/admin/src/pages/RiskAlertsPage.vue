  ...
  <section class="risk-page">
    <header class="page-heading">
      <div>
        ...
        <h1>风险告警</h1>
        <p>集中处置短时间窗独立失败激增。页面仅展示聚合证据，不展示 IP 或安装关联键。</p>
      </div>
    ...
    <form class="panel filters" @submit.prevent="loadAlerts(true)">
      ...
      <label class="query">告警或候选 ID<input v-model="query" maxlength="128" placeholder="输入完整 ID" /></label>
      <button class="button" :disabled="loading">{{ loading ? "加载中…" : "查询" }}</button>
    </form>
    ...
    <div class="risk-layout">
      <aside class="panel alert-list">
        <button v-for="item in alerts" :key="item.id" class="alert-row" :class="{ selected: selected?.id === item.id }" @click="openAlert(item.id)">
          <span><strong>失败激增</strong><b class="badge" :data-status="item.status">{{ statusLabels[item.status] }}</b></span>
          <code>{{ shortId(item.candidate_id) }}</code>
          <small>{{ item.independent_failure_count }} 个独立失败 · 权重 {{ item.failure_weight }} · {{ formatTime(item.created_at) }}</small>
        </button>
      ...
      <article v-if="selected" class="panel detail">
        ...
        <dl class="detail-grid">
          ...
          <div><dt>规则版本</dt><dd><code>{{ selected.rule_version }}</code></dd></div>
        ...
        <section><h3>不可变告警时间线</h3><ol class="timeline"><li v-for="event in selected.events" :key="event.id"><strong>{{ event.action }}</strong><p>{{ event.previous_status || "检测" }} → {{ event.next_status }} · {{ event.reason_code }}</p><p v-if="event.note">{{ event.note }}</p><small>{{ formatTime(event.created_at) }} · {{ event.request_id || "系统检测" }}</small></li></ol></section>
      </article>
...
.page-heading, .detail-title, .alert-row span, .actions { display: flex; justify-content: space-between; gap: 12px; align-items: center; }
...
.badge { padding: 4px 9px; border-radius: 999px; background: #e2e8f0; font-size: 12px; font-weight: 800; }.badge[data-status="open"] { background: #fef3f2; color: #b42318; }.badge[data-status="acknowledged"] { background: #fff7ed; color: #9a3412; }.badge[data-status="resolved"] { background: #ecfdf3; color: #067647; }
