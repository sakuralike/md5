import { isPrivilegedRole, type TokenResponse, type User } from "@password-detective/api-contract";
...
function readStoredSession(): { accessToken: string; user: User } | null {
  const stored = sessionStorage.getItem(STORAGE_KEY);
  if (!stored) return null;
  try {
    return JSON.parse(stored) as { accessToken: string; user: User };
  } catch {
...
export const useAdminAuthStore = defineStore("admin-auth", () => {
  ...
  async function logout(): Promise<void> {
    if (accessToken.value) {
      try { await apiRequest("/auth/logout", { method: "POST" }, accessToken.value); } catch { /* 清理本地状态 */ }
    }
    accessToken.value = "";
    user.value = null;
    sessionStorage.removeItem(STORAGE_KEY);
