...
import { DESKTOP_PLUGIN_CAPABILITIES, type DesktopPluginBuildProof, type DesktopPluginCapability, type DesktopPluginProjectDetail, type DesktopPluginStaticReviewReport, type DesktopPluginStaticReviewRun, type DesktopPluginVersion } from "@password-detective/api-contract";
...
import { attachPluginBuildProof, createDeveloperPlugin, createPluginVersion, finalizePluginVersion, getPluginReviewReport, listDeveloperPlugins, listPluginSigningKeys, registerPluginSigningKey, submitPluginVersion, uploadPluginPackage, withdrawPluginVersion, type SigningKeyRecord } from "../services/developerPlugins";
...
const buildProofVersionId = ref("none");
const buildProofArchitecture = ref<"windows-x64" | "windows-arm64">("windows-x64");
const buildProofGithubRunId = ref("");
const buildProofGithubArtifactId = ref("");
const buildProofJson = ref("");
...
const buildProofVersions = computed(() => projects.value
  .flatMap((project) => project.versions)
  .filter((version) => canAttachBuildProof(version)));
const selectedBuildProofVersion = computed(() => buildProofVersions.value.find(
  (version) => version.id === buildProofVersionId.value,
));
...
function canAttachBuildProof(version: DesktopPluginVersion): boolean {
  return ["source", "reproducible"].includes(version.source_review_mode)
    && ["quarantined", "review_queued", "auto_review_running", "auto_review_failed", "manual_review_ready", "approved"].includes(version.status);
}
...
function isBuildProof(value: unknown): value is DesktopPluginBuildProof {
  if (!isRecord(value)) return false;
  const provenance = value.provenance;
  return value.schema === "pd.plugin.build-proof/v1"
    && typeof value.git_commit === "string"
    && typeof value.package_sha256 === "string"
    && typeof value.rebuild_sha256 === "string"
    && value.content_reproducible === true
    && isRecord(provenance)
    && provenance.schema === "pd.plugin.provenance/v1"
    && Array.isArray(provenance.source_files)
    && isRecord(provenance.sbom)
    && Array.isArray(provenance.binaries)
    && isRecord(value.toolchain);
}

function parseBuildProof(raw: string): DesktopPluginBuildProof {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw) as unknown;
  } catch {
    throw new Error("构建证明必须是有效 JSON。");
  }
  if (!isBuildProof(parsed)) {
    throw new Error("构建证明缺少有效的 schema、提交、摘要或 provenance 字段。");
  }
  return parsed;
}
...
async function attachBuildProof(): Promise<void> {
  success.value = "";
  const version = selectedBuildProofVersion.value;
  if (!version) {
    error.value = "请选择可绑定构建证明的插件版本。";
    return;
  }
  let githubRunId: number;
  let githubArtifactId: number;
  let proof: DesktopPluginBuildProof;
  try {
    githubRunId = parsePositiveInteger(buildProofGithubRunId.value, "GitHub Run ID");
    githubArtifactId = parsePositiveInteger(buildProofGithubArtifactId.value, "GitHub Artifact ID");
    proof = parseBuildProof(buildProofJson.value);
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : "构建证明输入无效";
    return;
  }
  busy.value = true;
  error.value = "";
  try {
    await attachPluginBuildProof(
      ...
      githubRunId,
      githubArtifactId,
      buildProofArchitecture.value,
    );
    buildProofJson.value = "";
    buildProofGithubRunId.value = "";
    buildProofGithubArtifactId.value = "";
    success.value = `版本 ${version.semver} 的构建证明已提交服务端校验。`;
    await load();
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : "绑定构建证明失败";
  } finally {
    busy.value = false;
  }
}
...
<template>
  <section class="mx-auto w-full max-w-6xl space-y-6 px-4 py-8">
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(22rem,0.9fr)]">
      ...
      <div class="space-y-6">
        <Card><CardHeader><CardTitle>绑定 GitHub 构建证明</CardTitle><CardDescription>仅源码或可复现版本可绑定。服务端会重新校验仓库、运行、Artifact、提交和 provenance。</CardDescription></CardHeader><CardContent class="space-y-4"><div class="space-y-2"><Label for="build-proof-version">待绑定版本</Label><Select v-model="buildProofVersionId"><SelectTrigger id="build-proof-version"><SelectValue placeholder="选择插件版本" /></SelectTrigger><SelectContent><SelectItem value="none">选择插件版本</SelectItem><SelectItem v-for="version in buildProofVersions" :key="version.id" :value="version.id">{{ version.semver }} · {{ statusLabel(version.status) }}</SelectItem></SelectContent></Select></div><div class="grid gap-4 sm:grid-cols-2"><div class="space-y-2"><Label for="build-proof-run-id">GitHub Run ID</Label><Input id="build-proof-run-id" v-model="buildProofGithubRunId" inputmode="numeric" placeholder="例如 33744780271" /></div><div class="space-y-2"><Label for="build-proof-artifact-id">GitHub Artifact ID</Label><Input id="build-proof-artifact-id" v-model="buildProofGithubArtifactId" inputmode="numeric" placeholder="例如 9889337066" /></div></div><div class="space-y-2"><Label for="build-proof-architecture">制品架构</Label><Select v-model="buildProofArchitecture"><SelectTrigger id="build-proof-architecture"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="windows-x64">windows-x64</SelectItem><SelectItem value="windows-arm64">windows-arm64</SelectItem></SelectContent></Select></div><div class="space-y-2"><Label for="build-proof-json">build-proof.json</Label><Textarea id="build-proof-json" v-model="buildProofJson" rows="12" spellcheck="false" placeholder="粘贴 CI 生成的 build-proof.json 内容" /></div></CardContent><CardFooter><Button :disabled="busy" @click="attachBuildProof">{{ busy ? "绑定中…" : "绑定并校验构建证明" }}</Button></CardFooter></Card>
        <Card><CardHeader><CardTitle>我的插件项目</CardTitle><CardDescription>版本状态、自动审核报告和已批准权限。</CardDescription></CardHeader><CardContent class="space-y-3"><article v-for="project in projects" :key="project.id" class="space-y-3 rounded-md border p-4"><div class="flex items-start justify-between gap-3"><div><h2 class="font-medium">{{ project.name }}</h2><code class="text-xs text-muted-foreground">{{ project.slug }}</code></div><Badge>{{ project.status }}</Badge></div><div v-for="version in project.versions" :key="version.id" class="rounded-md bg-muted p-3 text-sm"><div class="flex justify-between gap-3"><strong>{{ version.semver }}</strong><div class="flex items-center gap-2"><Badge variant="outline">{{ statusLabel(version.status) }}</Badge><Button v-if="['quarantined','review_queued','auto_review_running','auto_review_failed','manual_review_ready','rejected'].includes(version.status)" size="sm" variant="ghost" :disabled="busy" @click="withdraw(version)">撤回</Button></div></div><p class="mt-2 text-xs text-muted-foreground">构建证明：{{ version.build_proof_verified_at ? "已验证" : "未绑定" }}<span v-if="version.build_proof_git_commit"> · {{ version.build_proof_git_commit.slice(0, 12) }}</span></p><div v-if="latestReview(version.id)" class="mt-3 space-y-2 border-t pt-3"><div class="flex items-center justify-between gap-2"><span class="text-xs font-medium">自动审核 {{ latestReview(version.id)?.policy_version }}</span><Badge :variant="latestReview(version.id)?.status === 'passed' ? 'default' : 'destructive'">{{ latestReview(version.id)?.status }}</Badge></div><div v-for="finding in latestReview(version.id)?.findings" :key="finding.id" class="rounded-md border bg-background p-2"><div class="flex items-center gap-2"><code class="text-xs">{{ finding.rule_id }}</code><Badge variant="outline">{{ finding.severity }}</Badge></div><p class="mt-1 text-xs">{{ finding.title }}</p><p class="text-xs text-muted-foreground">{{ finding.detail }}</p></div></div><p v-if="version.approved_capabilities.length" class="mt-2 text-xs text-muted-foreground">批准权限：{{ version.approved_capabilities.join('、') }}</p></div></article><p v-if="projects.length === 0" class="text-sm text-muted-foreground">尚无插件项目。</p></CardContent></Card>
      </div>
    </div>
  </section>
</template>
