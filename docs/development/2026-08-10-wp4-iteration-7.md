...
远端首轮恢复作业暴露 API 容器启动失败时缺少服务日志的问题。本轮补充两层收口：

- API Compose 启动在 Alembic 升级失败时最多重试 5 次，每次间隔 5 秒；全部失败后仍以非零状态退出。
- 恢复脚本启动前检查 API、MySQL 和 Redis 宿主机端口，避免误连其他本地服务；失败报告保存脱敏后的 Compose 状态、API 日志和 Worker 日志，并确保部分启动的环境也会清理。

本地隔离 Compose 恢复演练复验通过；远端结果以本轮最终提交对应的 GitHub Actions 为准。
