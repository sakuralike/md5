...
验证状态规则当前固定为 `verification-v3`，关联分析规则固定为 `correlation-v1`：游客和登录用户的 Web 提交均进入待验证池，至少 4 个独立登录用户成功反馈才自动验证；已登录且通过完整服务端校验的桌面成功回执可直接验证。规则版本会写入反馈历史、关联聚合快照和状态事件；修改阈值、可信通道或关联策略时必须发布新规则版本并补充迁移/回放测试，不能直接改写历史证据。
