...
| 管理端 | `apps/admin/` | 独立登录、角色检查、TOTP 登录/首次绑定、桌面发布草稿/上传重试/发布/撤回工作台 | M4 候选审核、用户处置与审计查询工作台 |
...
| API Contract | `packages/api-contract/` | 共享认证、浏览器会话、档案查询、贡献、揭示、反馈、桌面安装/挑战/回执、发布记录和证据快照类型 | 从 OpenAPI 自动生成并做契约差异检查 |
