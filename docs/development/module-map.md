...
| Archives | `apps/api/src/password_detective/modules/archives/` | 完整指纹精确查询、可见性分层、幂等贡献、重复候选合并、揭示配额和个人贡献列表 | 独立验证证据、状态事件、积分正式结算与高风险二次验证 |
...
| Database | `apps/api/src/password_detective/db/` | 用户、会话、账号动作令牌、幂等、审计、设置、档案、指纹、候选、贡献、积分流水模型 | 验证证据、状态事件和反馈模型 |
...
| 模块 | 路径 | 当前职责 | 下一步 |
|---|---|---|---|
| 用户 Web | `apps/web/` | 注册、登录、HttpOnly 刷新会话、本地分块哈希、精确查询、授权贡献、候选揭示和个人贡献 | Web Worker 隔离、验证反馈、超大文件性能与浏览器 E2E |
...
| API Contract | `packages/api-contract/` | 共享认证、浏览器会话、用户/会话、档案查询、贡献和揭示类型 | 从 OpenAPI 自动生成并做契约差异检查 |
