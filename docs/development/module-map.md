...
| Worker | `apps/api/src/password_detective/worker.py` | Celery 应用、探活、风险告警 SLA 扫描、社区通知 Outbox 扫描、统一通知网关、签名 Webhook、SMTP STARTTLS/SSL 投递、有限重试和失败终态记录 | SMTP 最终送达/退信回调、投递租约、多渠道路由、幂等记录清理、验证批处理和报表 |

## 客户端模块

| 模块 | 路径 | 当前职责 | 下一步 |
|---|---|---|---|
| 用户 Web | `apps/web/` | 注册、登录、HttpOnly 刷新会话、本地分块哈希、精确查询、授权贡献、候选揭示、社区验证反馈、社区论坛主题/回复、共享通知状态、通知中心实时插入/去重/已读同步、个人贡献、举报/申诉，以及积分/信誉/反馈历史中心 | Web Worker 隔离、通知多标签页同步、案件结果通知、超大文件性能与浏览器 E2E |
...
- 当前已完成固定分区、主题、回复、用户举报和 MFA 治理；论坛首页重构、点赞/收藏、公开主页、关注/粉丝、屏蔽/静音、可配置板块、群组、动态、社区搜索、通知和一对一私信按 `项目文档/密码侦探社社区完整功能开发计划-v1.0.md` 分轮推进。
- 后续实现统一使用 `modules/community/visibility.py` 作为可见性策略入口，以事实关系表加可重建投影保存互动计数，以事务事件/Outbox 驱动动态、通知和搜索；私信采用独立 AES-GCM 密钥环和最小披露治理。
