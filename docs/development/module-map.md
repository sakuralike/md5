...
| Risk Alerts | `apps/api/src/password_detective/modules/risk_alerts/` | `risk-alert-v1` 15 分钟失败激增检测、聚合告警投影、不可变事件、MFA 队列/详情和幂等处置 | 通知、SLA、批量操作与动态规则配置；关联分析由 Correlation 模块提供 |
...
| Database | `apps/api/src/password_detective/db/` | 用户、会话、账号动作令牌、幂等、审计、设置、档案、指纹、候选、贡献、积分、信誉事件、奖励校正事件、反馈、证据历史、关联分析快照、自动/人工状态事件、信任案件/案件事件、安装实例、挑战、桌面回执和桌面发布模型 | 案件 SLA、通知投递状态和跨候选关联模型 |
