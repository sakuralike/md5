...
| 桌面更新发布通道 | 完成首版 | 后端稳定/测试通道、MFA 草稿/上传/发布/撤回、制品摘要与大小校验、匿名检查/下载；桌面启动自动检查并显式打开系统浏览器下载入口 |
| 自动化门禁 | 本地代码门禁通过；容器待托管复验 | 后端 8 个 M3 专项测试、桌面 11 个安全/样本/更新协议测试；统一门禁与 Alembic 全量升降级往返通过。本地 Dockerfile/Compose 静态检查通过，但空环境构建多次受外部 PyPI/npm 索引超时阻塞，等待 PR #15 Container Images 复验 |
...
- [x] 本轮更新通道代码的统一门禁与 Alembic 全量升降级往返通过。
- [ ] 本轮 Docker 空环境门禁通过；Dockerfile/Compose 静态检查已通过，但 2026-08-02 的本地重建多次受外部 PyPI/npm 索引超时阻塞。
- [ ] PR #15 本轮 GitHub CI 的 API、Frontend、Desktop 和 Container Images 通过。
