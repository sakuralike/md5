...
- 更新日期：2026-08-03
- 当前里程碑：N2 管理员治理中心
- 最近迭代：[N2 第 4 次开发迭代：管理员危险操作安全链](./2026-08-03-n2-iteration-4.md)
...
> 2026-08-03 本轮 N2 第 4 次迭代已完成实现与统一本地门禁；API 89 项测试中 88 项通过、1 项真实 Redis 条件测试跳过，覆盖率 90.46%，共享契约 2 项、Web 19 项、Admin 29 项和桌面端 11 项测试通过。门禁通过仍不等于角色变更、配置治理、浏览器 E2E、真实 SMTP 送达或生产放行完成。

## 总体完成度快照

| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 全规格功能实现度 | 约 89% | M1～M3 主流程、M4 核心垂直切片和 N1 用户安全/隐私切片已形成；N2 已完成仪表盘、审计中心、用户治理总览和首批受控用户处置，但配置版本治理、角色变更、案件编排、浏览器 E2E 和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 75% | 统一本地门禁历史轮次和 1440 px Web 静态视觉复核通过，管理端已具备首批受控用户处置；配置治理、角色变更、浏览器交互与视觉回归 E2E、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
...
| M4 社区信任与管理闭环 | 约 83% | 五个 M4 垂直切片和 N2 仪表盘、审计中心、用户治理及首批受控用户处置完成；角色变更、系统配置、案件编排、账号申诉、结果通知和真实 SMTP 服务商最终送达验收待实现 |
...
- 本轮不新增数据表；用户治理、系统配置、管理员危险操作近期再认证、审计保留/归档和大规模异步导出继续列为 N2 后续范围。
...
| 管理处置闭环 | 受控用户处置完成 | 候选筛选、证据详情、状态时间线、隔离/恢复/通过/拒绝、用户停用/恢复、会话撤销、原因记录、幂等、并发和审计已实现；角色变更、批量处置、合并/删除和账号申诉待实现 |
