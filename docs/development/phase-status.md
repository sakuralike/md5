...
- 最近迭代：[WP3 第 2 次开发迭代：Web 查询、揭示、贡献与举报核心旅程](./2026-08-09-wp3-iteration-2.md)
- 前次迭代：[WP3 第 1 次开发迭代：Playwright 基础设施与认证核心旅程](./2026-08-09-wp3-iteration-1.md)
...
> 2026-08-09 WP3 第 2 次迭代在第 1 次认证门禁之上，新增真实 FastAPI/Vite 服务下的 Web 已验证查询、揭示与页面清除、授权贡献、候选举报和本人案件时间线三条 Chromium 闭环；隔离种子仅使用合成账号、指纹和加密候选字段。上一轮远端 GitHub Actions 运行 `31290899019` 的 API、Frontend、Desktop、E2E 和 Container Images 已全部通过，本轮推送后的 CI 仍需复核。Web 邮箱验证/刷新过期、用户安全、Admin 案件处置、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
