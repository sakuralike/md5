...
- 更新日期：2026-08-03
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 6 次开发迭代](./2026-08-03-m4-iteration-6.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 90% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分信誉、奖励补偿、失败激增告警和候选内关联降权已形成；跨候选图谱、用户处置、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 76% | 自动化门禁、M4 审核/案件/信誉/补偿/告警/关联降权专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
| 生产上线准备度 | 约 45% | 容器、MySQL/SQLite 迁移、CI、不可变奖励校正、应用内风险告警和关联聚合基线已建立；外部通知、监控平台、安全扫描、恢复演练、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 89% | 候选审核、举报/申诉、积分投影、不可变信誉事件、奖励补偿、失败激增告警和候选内关联降权首版完成；跨候选图谱、用户处置和案件编排待实现 |
...
| 异常检测和告警 | 两个垂直切片完成 | `risk-alert-v1` 与 `correlation-v1` 已实现失败激增检测、候选内关联降权、聚合最小披露、不可变事件/快照和 MFA 管理处置；跨候选分析、通知、SLA 和并发任务化待实现 |
