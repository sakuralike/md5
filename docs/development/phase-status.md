...
- 最近迭代：[WP4 第 4 次开发迭代：认证对象边界与浏览器 Cookie 安全门禁](./2026-08-10-wp4-iteration-4.md)
...
> 2026-08-10 WP4 第 4 次迭代在隔离端口执行 10 项确定性动态安全探针，关闭已认证普通用户隐私导出读取/会话撤销的跨用户对象边界，以及浏览器刷新 Cookie 的 HttpOnly、SameSite、路径隔离和来源校验。提交 `afba67a` 的远端 GitHub Actions `31349411600` 九个作业全部通过。管理员 MFA/再认证、幂等重放、资源消耗、人工渗透、恢复、性能、可观测性和目标环境仍未关闭。
...
| MVP 验收完成度 | 约 95% | 三浏览器 90 项旅程、依赖/SAST/SBOM、最终镜像、仓库 Secret、风险接受和匿名/普通用户认证 API 动态基线已形成自动门禁；管理员认证滥用、Edge、真实 Safari、NVDA/VoiceOver、真实移动设备、恢复演练、规模压测、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
| 生产上线准备度 | 约 59% | 容器、迁移、CI、依赖/SAST/SBOM、最终镜像与 Secret 阻断、风险接受和匿名/普通用户认证 DAST 已建立；管理员认证滥用、人工渗透、发件域名、指标出口、监控平台、恢复演练、密钥轮换、合规和灰度发布尚未闭环 |
...
| M5 稳定、合规与发布 | 约 45% | 三浏览器旅程、依赖/SAST/SBOM、最终镜像与 Secret 阻断、限期风险接受、提交级证据和匿名/普通用户认证 API 动态基线已建立；管理员认证滥用、人工渗透、恢复、性能、监控、RC 和灰度未实施 |
