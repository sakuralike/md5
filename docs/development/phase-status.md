...
| 自动化门禁 | 本地代码与托管 CI 通过 | 后端 8 个 M3 专项测试、桌面 11 个安全/样本/更新协议测试；统一门禁、Alembic 全量升降级往返及 PR #15 的 API、Frontend、Desktop、Container Images 全部通过。本地 Dockerfile/Compose 静态检查通过；空环境重建仍受外部 PyPI/npm 索引超时阻塞 |
