...
- 更新日期：2026-08-16
- 当前执行切片：[WP5-I9 私信浏览器验收与统一门禁](./2026-08-16-wp5-i9-browser-e2e.md)；本地三浏览器、统一门禁、远端等价树同步和 Staging 部署/独立复验已完成。
- 当前里程碑：WP5-I9 异步一对一私信基础收口
...
- [x] 游客/登录用户 Web 提交进入待验证池，4 个独立登录用户成功反馈自动验证；可信桌面成功回执直接验证；三个独立失败反馈自动隔离并暂停揭示。
...
| 证据接入 | 完成 `verification-v3` | 游客/登录用户 Web 提交统一进入待验证池并要求 4 个独立登录用户成功反馈；通过服务端完整校验的登录桌面成功回执可直接验证；共享安装/IP 的候选内反馈继续由 `correlation-v1` 动态限权 |
...
- `verification-v3` 消费关联降权后的有效权重与独立组数，Web 自动验证阈值提升为 4 个独立成功组，并允许经服务端完整校验的登录桌面成功回执直接验证；每次有效反馈材料变化在同一事务内追加不可变聚合快照。
...
## 2026-09-03 桌面插件 CI 证明与迁移重试收口

| 范围 | 状态 | 说明 |
|---|---|---|
| GitHub 构建证明 | 已完成（真实 CI 已验证） | CI run `33665356191` 的 `plugin build proof` 与重跑后的 `desktop` job 均成功；artifact `9860474083` 已由服务端真实下载并校验 proof 清单摘要、commit `24ed87e...` 和 provenance 内容摘要 |
...
## 2026-08-12 WP5 社区完整功能规划基线

| 规划范围 | 计划文档 | 当前状态 | 下一入口 |
|---|---|---|---|
| 论坛首页、帖子与评论重构 | `项目文档/密码侦探社社区完整功能开发计划-v1.0.md` WP5-I3 | 业务编码与本地自动化已收口 | 当前提交的目标 MySQL 与浏览器回归 |
| 点赞、收藏、关注、粉丝、公开主页 | 同上 WP5-I4～I5 | WP5-I4 COMMUNITY-32～35、WP5-I5 COMMUNITY-37～44 已完成本地编码与自动化 | 点赞摘要、关注通知留待 I7 |
| 可配置板块和社区群组 | 同上 WP5-I6 | COMMUNITY-45～54 本地代码与自动化已完成 | 目标 MySQL 迁移、真实浏览器回归；下一轮进入 I7 动态与通知 |
| 动态与通知 | 同上 WP5-I7 | 动态信息流已完成本地编码；通知中心仍为已实现子集 | 点赞/群组/私信与治理通知、邮件投递、SSE 和目标环境回归 |
| 社区搜索 | 同上 WP5-I8 | COMMUNITY-62～68 已完成本地代码、迁移、Web/Admin 页面、定向跨浏览器旅程和统一门禁 | 目标 MySQL `ngram`/性能与降级演练、Staging、远端等价树和发布验收 |
...
| 需求 | 实现证据 | 自动化证据 | 当前状态与剩余风险 |
|---|---|---|---|
| 作者逻辑删除回复 | `community.service.delete_comment` 同事务扣减主题 `reply_count`，重复删除返回 `community.comment_already_deleted` | `test_author_comment_delete_updates_reply_count_once` | 已修复未扣减和重复删除风险；计数下限保持为 0 |
| 治理移除作者删除占位 | `community.admin_service._remove_reported_content` 仅对仍计入公开回复数的评论扣减 | `test_moderating_author_deleted_comment_does_not_double_decrement` | 已修复治理二次扣减风险 |
...
| 需求 | 实现证据 | 自动化/环境证据 | 当前状态 |
|---|---|---|---|
| `20260812_0028` MySQL 兼容 | MySQL 使用自连接更新，其他方言保留相关子查询 | `test_migration_0028_mysql.py` | 已实现 |
| 非事务 DDL 失败重入 | 迁移反射并跳过已存在字段、表、索引和外键 | 半执行目标库重试后 Alembic 到 `20260812_0028` | 已验证 |
...
## 2026-08-12 WP5-I6 第 1 个开发切片：可配置板块与群组治理

| 需求 | 实现证据 | 自动化证据 | 当前状态与剩余风险 |
|---|---|---|---|
| 可配置板块 | `community_boards`、动态目录、最小角色/只读/停用状态、Admin `/community/settings` | `test_community_board_admin.py`、`CommunityConfigurationPage.test.ts` | 本地已实现；目标 MySQL 与浏览器回归待部署 |
| 群组成员与角色 | `community_groups`、`community_group_memberships`、加入/审批/邀请/退出/移除/角色/群主转移 API | `test_community_groups.py` | 本地已实现；大规模成员分页和运营策略留待容量阶段 |
| 私密内容边界 | 统一主题读取、公开主页和提及通知过滤；全站治理不自动旁路普通读取 | 私密列表/详情/主页/通知与全站版主负向测试 | 本地已验证；I7～I8 的动态、搜索必须继续复用同一策略 |
| Web 群组入口 | `/community/groups`、`/community/groups/:groupSlug`、发布页动态目标 | 群组页面与发布页渲染测试、Web typecheck/lint/Vitest | 本地已实现；真实浏览器主旅程待部署回归 |
...
### 2026-08-14 M5 入口治理与公告体验切片

本轮完成总哈希池 Admin 查询、用户审批手动创建用户、用户等级与权益入口迁移、Web 右下角公告弹窗，以及 Windows 公告请求的 API 基址和缓存修复。专项 API/Admin/Web/Windows 测试与统一门禁均通过；功能树 `98ff447813a9` 已发布至 Staging，公网 Web/Admin/ready/桌面公告冒烟通过。

当前剩余项：真实浏览器公告与已登录管理旅程、桌面 Release 安装包实机验证、邮件摘要和多实例通知能力、HA/容量/恢复与正式回滚演练。


## 2026-08-14 R3 第 3 轮：桌面公告与 Web 公告独立化

- 新增 `web_announcements` 独立模型、迁移、公开/管理 API 和图片目录；桌面公告链路保持原状。
- Admin 增加“Web 公告”独立入口与 0～86400 秒自动关闭配置；Web 弹窗不再读取桌面公告。
- 自动计时关闭只影响当前页面，手动关闭仍按公告修订持久化。
- 专项 API、Admin、Web 测试及 `pwsh ./scripts/check.ps1 -SkipInstall` 统一门禁通过；提交、远端和 Staging 证据将在本轮结束时回填。

## 2026-08-25 桌面插件市场第 2 轮：控制面与制品存储
...
## 2026-08-25 桌面插件市场第 3 轮：Web 开发者中心与 Admin 人工审核

| 层级 | 当前证据 | 状态 |
|---|---|---|
| Web 开发者中心 | `/developer/plugins` 项目、密钥、版本、上传、第三方应用关联和提交工作流 | 本地已实现；Web lint/typecheck 与 58 文件 103 项测试通过 |
...
- 远端已推送：待本轮提交、远端 ref/tree/commit 对比完成后回填。
- Staging 已部署：待本轮服务器迁移、重启和 HTTP 检查完成后回填。
- UAT 已验收：未执行；需 Web 页面、三浏览器旅程和目标环境验证。
- 剩余风险：Web 私信入口/页面、SSR/首屏、Chromium/Firefox/WebKit、统一门禁、MySQL 迁移和服务器回滚证据仍未完成。


## 2026-08-16 WP5-I10 私信实时事件本地实现

| 层级 | 当前证据 | 状态 |
|---|---|---|
| 数据与 API | Alembic `20260816_0045`、每用户持久事件序号、三类事件、独立 SSE、Redis 唤醒和数据库补偿已实现；五组定向 API/迁移/加密测试通过 | 本地已实现 |
| Web | 独立流服务/Store/生命周期已接入账户菜单、收件箱和会话页；Web 45 文件 78 测试通过 | 本地已实现 |
