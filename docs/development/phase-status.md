...
- 最近迭代：[WP3 第 5 次开发迭代：Admin 用户治理与角色双人审批闭环](./2026-08-09-wp3-iteration-5.md)
- 前次迭代：[WP3 第 4 次开发迭代：Web 用户安全与隐私浏览器闭环](./2026-08-09-wp3-iteration-4.md)
...
> 2026-08-09 WP3 第 5 次迭代新增真实 FastAPI/Vite 服务下的 Admin 普通用户停用/恢复与活跃会话撤销、不同管理员角色变更申请/批准两条 Chromium 闭环。隔离种子只使用合成管理员、用户、TOTP 和不可登录刷新哈希；定向检查与包含 14 项 Chromium 旅程的统一本地门禁已通过，远端 GitHub Actions `31298489273` 的 API、Frontend、Desktop、E2E 和 Container Images 五个作业全部通过。系统配置、风险告警、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
