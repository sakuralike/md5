...
- 最近迭代：[WP4 第 3 次开发迭代：API 动态安全与负向门禁](./2026-08-09-wp4-iteration-3.md)
...
> 2026-08-09 WP4 第 3 次迭代增加 API 安全响应头、默认 1 MiB JSON 请求体限制和分块绕过阻断，并在隔离端口执行 8 项确定性动态安全探针。提交 `45af0d3` 的远端 GitHub Actions `31340260242` 九个作业全部通过，下载报告为 `total=8`、`passed=8`、`failed=0`。这关闭的是匿名 API DAST 基线；认证 BOLA、Cookie CSRF、MFA/再认证与幂等重放、资源消耗、人工渗透、恢复、性能、可观测性和目标环境仍未关闭。
...
| MVP 验收完成度 | 约 94% | 三浏览器 90 项旅程、依赖/SAST/SBOM、最终镜像、仓库 Secret、风险接受与匿名 API 动态基线已形成自动门禁；Edge、真实 Safari、NVDA/VoiceOver、真实移动设备、认证安全矩阵、恢复演练、规模压测、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
| 生产上线准备度 | 约 57% | 容器、迁移、CI、依赖/SAST/SBOM、最终镜像与 Secret 阻断、风险接受和匿名 API 动态基线已建立；认证 DAST、人工渗透、发件域名、指标出口、监控平台、恢复演练、密钥轮换、合规和灰度发布尚未闭环 |
...
| M5 稳定、合规与发布 | 约 40% | 三浏览器旅程、依赖/SAST/SBOM、最终镜像与 Secret 阻断、限期风险接受、提交级证据和匿名 API 动态基线已建立；认证安全矩阵、人工渗透、恢复、性能、监控、RC 和灰度未实施 |
...
| 管理处置闭环 | 普通角色双人审批完成 | 候选筛选、证据详情、状态时间线、隔离/恢复/通过/拒绝、用户停用/恢复、会话撤销、普通角色变更申请/双人复核、原因记录、幂等、并发和审计已实现；独立角色事件时间线、批量处置、合并/删除和案件原子处置待实现 |
| 举报、信誉、申诉和配置 | 部分完成 | 举报/申诉统一案件及管理队列、账号申诉基线、指派/重开和信誉/补偿事件已实现；结果通知、原子案件处置、SLA 和 Web 账号申诉交互待实现 |
