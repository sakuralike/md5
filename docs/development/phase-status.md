...
- 更新日期：2026-08-03
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 8 次开发迭代](./2026-08-03-m4-iteration-8.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 92% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分信誉、奖励补偿、失败激增、关联降权、告警 SLA/指派、签名通知与失败重放闭环已形成；跨候选图谱、用户处置、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 80% | 自动化门禁、M4 审核/案件/信誉/补偿/告警/关联/SLA/通知投递专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测、真实通知端点和生产密钥验收未完成 |
| 生产上线准备度 | 约 50% | 容器、MySQL/SQLite 迁移、CI、不可变奖励校正、告警 SLA、签名 Webhook、投递指标和死信重放基线已建立；真实提供商验收、指标导出、监控平台、安全扫描、恢复演练、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 95% | 候选审核、举报/申诉、积分投影、不可变信誉事件、奖励补偿、失败激增、候选内关联降权，以及告警 SLA/指派/签名通知/死信重放闭环完成；跨候选图谱、用户处置、案件编排和真实通知环境验收待实现 |
...
- 自动化已覆盖三次失败终态、指标/过滤、最小披露、幂等重放、审计原因、规范载荷、签名和提供商回执；完整本地门禁与 CI 结果见本轮迭代记录。
...
| 异常检测和告警 | 四个垂直切片完成 | `risk-alert-v1`、`correlation-v1`、`risk-alert-sla-v1` 与 `notification-webhook-v1` 已实现失败激增、候选内关联降权、MFA 值班指派、签名通知、投递指标、失败队列和幂等人工重放；真实提供商环境验收、排班升级链、指标导出和跨候选分析待实现 |
