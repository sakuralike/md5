...
- 更新日期：2026-08-02
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 2 次开发迭代](./2026-08-02-m4-iteration-2.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 82% | M1、M2、M3 主流程以及 M4 候选审核、举报和申诉首版已形成；信誉、异常告警、用户处置和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 66% | 自动化门禁、M4 审核及案件专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
...
| M4 社区信任与管理闭环 | 约 60% | 候选审核、用户举报/申诉、私有案件列表、MFA 管理队列、不可变事件、幂等和审计首版完成；信誉、告警、用户处置和案件联动待实现 |
...
| 举报、信誉、申诉和配置 | 部分完成 | 举报/申诉统一案件、用户入口、本人案件列表和管理队列已实现；信誉事件、反馈/积分/信誉页面、配置版本和结果通知待实现 |
