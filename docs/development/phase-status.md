...
- 最近迭代：[WP4 第 5 次开发迭代：管理员 MFA、再认证与幂等滥用门禁](./2026-08-10-wp4-iteration-5.md)
...
> 2026-08-10 WP4 第 5 次迭代在隔离端口执行 13 项确定性动态安全探针，新增管理员 MFA/再认证、幂等精确重放/冲突、一次性授权重放和限流退避门禁。远端等价提交 `4a96db42` 的 GitHub Actions `31352246919` 全部通过；本地功能提交为 `eb9fa79`。更完整对象矩阵、人工渗透、恢复、性能、可观测性和目标环境仍未关闭。
...
| MVP 验收完成度 | 约 96% | 三浏览器 90 项旅程、依赖/SAST/SBOM、最终镜像、仓库 Secret、风险接受、匿名/普通用户对象边界和管理员 MFA/再认证/幂等动态门禁已形成自动门禁；Edge、真实 Safari、NVDA/VoiceOver、真实移动设备、恢复演练、规模压测、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
| 生产上线准备度 | 约 61% | 容器、迁移、CI、依赖/SAST/SBOM、最终镜像与 Secret 阻断、风险接受和管理员认证动态门禁已建立；完整 BOLA、人工渗透、发件域名、指标出口、监控平台、恢复演练、密钥轮换、合规和灰度发布尚未闭环 |
...
| M5 稳定、合规与发布 | 约 50% | 三浏览器旅程、依赖/SAST/SBOM、最终镜像与 Secret 阻断、限期风险接受、提交级证据和匿名/普通用户及管理员认证动态基线已建立；人工渗透、恢复、性能、监控、RC 和灰度未实施 |
