...
- 更新日期：2026-08-02
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 4 次开发迭代](./2026-08-02-m4-iteration-4.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 86% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分信誉中心和奖励补偿已形成；异常告警、用户处置、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 71% | 自动化门禁、M4 审核/案件/信誉/补偿专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
| 生产上线准备度 | 约 43% | 容器、MySQL/SQLite 迁移、CI 和不可变奖励校正基线已建立；安全扫描、恢复演练、监控告警、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 78% | 候选审核、举报/申诉、积分投影、不可变信誉事件、奖励补偿和用户中心首版完成；告警、用户处置和案件编排待实现 |
...
| 页面原型/可运行界面 | 可运行增量 | Web 已覆盖认证、查询、贡献和用户安全；Admin 已覆盖认证、候选审核详情/处置、审计骨架和桌面发布 |
