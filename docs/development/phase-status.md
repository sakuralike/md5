...
- 最近迭代：[WP3 第 8 次开发迭代：视觉、可访问性与刷新竞争收口](./2026-08-09-wp3-iteration-8.md)
- 前次迭代：[WP3 第 7 次开发迭代：Web 邮箱、刷新令牌与移动端键盘门禁](./2026-08-09-wp3-iteration-7.md)
- 前序迭代：[WP3 第 6 次开发迭代：系统配置与风险告警浏览器闭环](./2026-08-09-wp3-iteration-6.md)
...
| MVP 验收完成度 | 约 89% | 统一本地门禁历史轮次、1440 px Web 静态视觉复核和首批结构化视觉/可访问性门禁通过，管理端已具备用户处置、配置治理和普通角色双人审批，Web/Admin 25 项核心浏览器旅程通过；跨平台像素差异、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
