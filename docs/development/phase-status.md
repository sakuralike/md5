...
- 最近迭代：[M3 第 1 次开发迭代](./2026-08-02-m3-iteration-1.md)
...
| 模块 | 状态 | 说明 |
|---|---|---|
| ZIP/7z 本地验证 | 核心完成 | 目录枚举、受控内容读取、30 秒超时、取消、8 GiB 文件、10,000 条目、32 GiB 展开大小、压缩比和路径安全限制 |
| 安装身份与会话 | 完成首版 | 首次启动 P-256 密钥；私钥和桌面令牌使用当前用户 DPAPI 保护；不采集硬件标识 |
| 桌面协议闭环 | 完成首版 | 登录、安装公钥注册、一次性挑战、规范载荷、ECDSA 签名和回执提交 |
| 服务端回执校验 | 完成首版 | 安装撤销、最低版本、挑战过期/单次使用、绑定、时钟偏差、签名与重放校验 |
| 证据接入 | 完成首版 | 桌面回执接入 `verification-v1`，同结果可升级 Web 证据来源，双独立安装可触发验证 |
| 自动化门禁 | 核心完成 | 后端 5 个协议/状态测试、桌面 3 个安全测试、迁移往返、统一门禁、Docker/MySQL 空环境冒烟及 PR #15 GitHub CI 全部通过 |
...
- [ ] 补充带密码 ZIP 与 7z 的成功/失败代表样本，以及损坏、超大和高压缩比实机样本矩阵。
- [ ] 完成安装撤销后的客户端重新注册引导和明确错误 UX。
- [x] PR #15 GitHub Windows CI 的桌面 Release 构建与 3 个安全测试通过。
