...
> 2026-08-09 WP3 第 4 次迭代新增真实 FastAPI/Vite 服务下的 Web 密码修改、其他会话撤销、TOTP 启停与登录门禁、隐私用途确认、个人数据一次性导出，以及账号删除请求创建/撤销三条 Chromium 闭环。隔离种子使用固定合成账号、不可登录的刷新哈希和独立 SQLite，且不改变生产 `auth.login` 速率限制。定向检查和包含 12 项 Playwright Chromium 旅程的统一本地门禁已通过；远端 CI 待本轮推送后复核。Admin 用户治理/角色审批/配置/风险告警、Web 邮箱刷新组合、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
