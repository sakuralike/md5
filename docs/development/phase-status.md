...
本轮完成总哈希池 Admin 查询、用户审批手动创建用户、用户等级与权益入口迁移、Web 右下角公告弹窗，以及 Windows 公告请求的 API 基址和缓存修复。专项 API/Admin/Web/Windows 测试均通过；统一门禁、Staging 发布和真实浏览器交互仍是本轮最终交付前置条件。

当前剩余项：真实浏览器公告与管理旅程、桌面 Release 安装包实机验证、邮件摘要和多实例通知能力、HA/容量/恢复验收、统一门禁和 Staging 发布证据。
