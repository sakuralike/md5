...
- 最近迭代：[WP3 第 6 次开发迭代：系统配置与风险告警浏览器闭环](./2026-08-09-wp3-iteration-6.md)
- 前次迭代：[WP3 第 5 次开发迭代：Admin 用户治理与角色双人审批闭环](./2026-08-09-wp3-iteration-5.md)
- 前序迭代：[WP3 第 4 次开发迭代：Web 用户安全与隐私浏览器闭环](./2026-08-09-wp3-iteration-4.md)
...
> 2026-08-09 WP3 第 6 次迭代新增真实 FastAPI/Vite 服务下的系统配置草稿/发布/历史版本回滚，以及风险告警指派/核查/解决两条 Chromium 闭环；同时修复写操作成功提示在详情刷新时被清空的问题。隔离种子只使用合成候选、证据、告警、管理员和 TOTP；Admin 51 项测试、完整 16 项 Chromium 旅程与统一本地门禁已通过，远端 GitHub Actions `31299937196` 的 API、Frontend、Desktop、E2E 和 Container Images 五个作业全部通过。Web 邮箱/刷新异常组合、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
