...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 全规格功能实现度 | 约 82% | M1～M3 主流程以及 M4 候选审核、举报/申诉、积分信誉、奖励补偿、失败激增、关联降权、告警 SLA/指派、签名 Webhook、SMTP 邮件与失败重放闭环已形成；用户中心隐私能力、管理员治理菜单、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 68% | 自动化门禁、M4 垂直切片专项测试稳定，但用户中心/管理治理仍不完整，浏览器 E2E、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
...
| M2 核心 Web 查询与贡献 | 约 78% | 业务闭环首版完成，用户中心和浏览器 E2E、规模性能和生产 KMS 未验收 |
