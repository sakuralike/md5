...
- 更新日期：2026-08-02
- 当前里程碑：M3 收尾与 M4 启动准备
- 最近迭代：[M3 第 4 次开发迭代](./2026-08-02-m3-iteration-4.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 75% | M1、M2、M3 主流程代码已形成；M4 管理闭环和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 60% | 自动化门禁稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
| 生产上线准备度 | 约 40% | 容器、迁移和 CI 基线已建立；安全扫描、恢复演练、监控告警、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 20% | 状态机、证据和积分已有基础，管理处置、举报、信誉、申诉和告警未实现 |
...
| 页面原型/可运行界面 | 可运行增量 | Web 已覆盖认证、查询、贡献和用户安全；Admin 已覆盖认证、审计骨架和桌面发布，M4 审核详情仍待实现 |
...
| 自动化门禁 | 本地代码、MySQL 空环境和全量往返通过，托管复跑中 | 后端 M3 专项测试、桌面 11 个安全/样本/更新协议测试、Admin 7 个服务测试通过；MySQL 8.4 空环境发现并修复 `release_notes` TEXT 默认值兼容问题；新增 CI 首轮又发现外键索引降级顺序问题，调整为直接删表后本地 MySQL 全量升降级往返通过 |
...
- [ ] 托管 CI MySQL 8.4 全量升降级往返通过；首轮运行 `30745274582` 暴露外键索引降级顺序问题，修复后的本地 MySQL 往返已通过，待托管复跑。
...
| 积分结算基础 | 部分完成 | 首次验证结算和幂等账本已实现；回滚补偿、余额重算和完整信誉事件待实施 |
| 管理处置闭环 | 未实施 | 候选筛选、证据详情、隔离/恢复/拒绝、用户处置和原因记录待实现 |
| 举报、信誉、申诉和配置 | 未实施 | reports、reputation_events、用户反馈/信誉/申诉页面、配置版本待实现 |
| 异常检测和告警 | 未实施 | 关联账号降权、失败激增告警、并发状态任务门禁待实现 |
