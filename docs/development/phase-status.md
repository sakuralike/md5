...
- 更新日期：2026-08-02
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 3 次开发迭代](./2026-08-02-m4-iteration-3.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 84% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分与信誉中心已形成；异常告警、用户处置、补偿规则和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 69% | 自动化门禁、M4 审核/案件/信誉专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
| 生产上线准备度 | 约 42% | 容器、MySQL/SQLite 迁移和 CI 基线已建立；安全扫描、恢复演练、监控告警、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 70% | 候选审核、举报/申诉、积分投影、不可变信誉事件和用户中心首版完成；补偿规则、告警、用户处置和案件联动待实现 |
...
| 积分与信誉 | 首版完成 | 首次验证积分结算、账本余额投影、初始 50 分、`reputation-v1` 和不可变信誉事件已实现；候选撤销后的补偿/扣减规则待实施 |
| 管理处置闭环 | 首版完成 | 候选筛选、证据详情、状态时间线、隔离/恢复/通过/拒绝、原因记录、幂等和审计已实现；用户处置、合并/删除和危险操作二次确认待实现 |
| 举报、信誉、申诉和配置 | 部分完成 | 举报/申诉统一案件及管理队列、信誉事件、反馈/积分/信誉页面已实现；配置版本、结果通知、账号申诉和案件联动待实现 |
