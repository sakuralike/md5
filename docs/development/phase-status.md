...
- 更新日期：2026-08-03
- 当前里程碑：N2 管理员治理中心
- 最近迭代：[N2 第 1 次开发迭代：真实治理仪表盘](./2026-08-03-n2-iteration-1.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 全规格功能实现度 | 约 84% | M1～M3 主流程、M4 核心垂直切片和 N1 用户安全/隐私切片已形成；N2 已完成真实仪表盘首个切片，但审计导出、用户治理、配置、案件编排、浏览器 E2E 和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 70% | 统一本地门禁和 1440 px Web 静态视觉复核通过，管理端仪表盘已使用真实数据；审计/用户/配置治理、浏览器交互与视觉回归 E2E、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
...
| M4 社区信任与管理闭环 | 约 75% | 五个 M4 垂直切片和 N2 真实仪表盘首个切片完成；用户处置、审计查询/导出、系统配置、案件编排、账号申诉、结果通知和真实 SMTP 服务商最终送达验收待实现 |
...
| 页面原型/可运行界面 | 可运行增量 | Web 已覆盖认证、查询、贡献和用户安全；Admin 已覆盖真实仪表盘、认证、候选审核、举报申诉、风险告警、审计骨架和桌面发布 |
...
- 本轮不新增数据表；审计查询/CSV 导出、用户治理、系统配置和管理员危险操作再认证继续列为 N2 后续范围。
