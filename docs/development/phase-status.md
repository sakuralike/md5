...
- 最近迭代：[WP3 第 4 次开发迭代：Web 用户安全与隐私浏览器闭环](./2026-08-09-wp3-iteration-4.md)
- 前次迭代：[WP3 第 3 次开发迭代：Admin 候选审核与案件原子处置](./2026-08-09-wp3-iteration-3.md)
- 前序迭代：[WP2 第 4 次开发迭代：SLA 自动升级与 Web 账号申诉](./2026-08-08-wp2-iteration-4.md)
...
> 2026-08-09 WP3 第 4 次迭代新增真实 FastAPI/Vite 服务下的 Web 密码修改、其他会话撤销、TOTP 启停与登录门禁、隐私用途确认、个人数据一次性导出，以及账号删除请求创建/撤销三条 Chromium 闭环。隔离种子使用固定合成账号、不可登录的刷新哈希和独立 SQLite，且不改变生产 `auth.login` 速率限制。定向检查和包含 12 项 Playwright Chromium 旅程的统一本地门禁已通过；远端 GitHub Actions `31296867172` 的 API、Frontend、Desktop、E2E 和 Container Images 五个作业全部通过。Admin 用户治理/角色审批/配置/风险告警、Web 邮箱刷新组合、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
...
| MVP 验收完成度 | 约 86% | 统一本地门禁历史轮次和 1440 px Web 静态视觉复核通过，管理端已具备用户处置、配置治理和普通角色双人审批；浏览器核心业务与视觉回归 E2E、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
