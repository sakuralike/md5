...
- 更新日期：2026-08-02
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 5 次开发迭代](./2026-08-02-m4-iteration-5.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 88% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分信誉、奖励补偿和失败激增告警已形成；关联账号、用户处置、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 73% | 自动化门禁、M4 审核/案件/信誉/补偿/告警专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测和生产密钥验收未完成 |
| 生产上线准备度 | 约 44% | 容器、MySQL/SQLite 迁移、CI、不可变奖励校正和应用内风险告警基线已建立；外部通知、监控平台、安全扫描、恢复演练、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 84% | 候选审核、举报/申诉、积分投影、不可变信誉事件、奖励补偿、用户中心和失败激增告警首版完成；关联账号降权、用户处置和案件编排待实现 |
...
- GitHub PR 与 CI 交付信息将在本分支推送和工作流完成后补记。

### 后续范围

| 范围 | 状态 | 说明 |
|---|---|---|
| 证据、反馈、状态事件基础 | 部分完成 | `vote_feedback`、证据历史、状态事件、独立关联键和自动晋级/隔离已在 M2/M3 建立 |
...
| 异常检测和告警 | 首个垂直切片完成 | `risk-alert-v1` 已实现 15 分钟独立失败激增检测、聚合最小披露、不可变事件和 MFA 管理处置；关联账号降权、通知、SLA 和并发任务化待实现 |
