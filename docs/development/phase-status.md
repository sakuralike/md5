...
| 自动化门禁 | 本地代码、MySQL 空环境与已提交 CI 通过 | 后端 M3 专项测试、桌面 11 个安全/样本/更新协议测试、Admin 7 个服务测试通过；MySQL 8.4 空环境发现并修复 `release_notes` TEXT 默认值兼容问题后，Compose 冒烟在约 0.9 分钟完成 |
...
- [x] Docker MySQL 8.4 空环境完成全量迁移、服务健康检查和合成注册/登录；迁移兼容缺陷已修复并增加托管 CI MySQL 往返门禁。
