...
> 2026-08-09 WP3 第 5 次迭代新增真实 FastAPI/Vite 服务下的 Admin 普通用户停用/恢复与活跃会话撤销、不同管理员角色变更申请/批准两条 Chromium 闭环。隔离种子只使用合成管理员、用户、TOTP 和不可登录刷新哈希；定向检查与包含 14 项 Chromium 旅程的统一本地门禁已通过，远端 CI 待本轮推送后复核。系统配置、风险告警、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
