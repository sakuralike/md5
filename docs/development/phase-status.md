...
> 2026-08-09 WP3 第 7 次迭代新增真实 FastAPI/Vite 服务下的邮箱待验证/一次性验证/重放拒绝、刷新令牌轮换/旧令牌重放/令牌族撤销，以及 390 × 844 移动视口无横向溢出和键盘返回登录三条 Chromium 门禁。隔离种子使用合成用户、凭证摘要和刷新会话，未放宽生产登录限流；完整 19 项 Chromium 旅程与统一本地门禁已通过，远端 GitHub Actions 待本轮提交后补记。视觉快照、更多页面移动端/可访问性、目标环境和生产验收仍未关闭。
