...
> 2026-08-09 WP3 第 2 次迭代在第 1 次认证门禁之上，新增真实 FastAPI/Vite 服务下的 Web 已验证查询、揭示与页面清除、授权贡献、候选举报和本人案件时间线三条 Chromium 闭环；隔离种子仅使用合成账号、指纹和加密候选字段。本轮远端 GitHub Actions 运行 `31292352350` 的 API、Frontend、Desktop、E2E 和 Container Images 已全部通过。Web 邮箱验证/刷新过期、用户安全、Admin 案件处置、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
