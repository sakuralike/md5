...
> 2026-08-09 WP3 第 6 次迭代新增真实 FastAPI/Vite 服务下的系统配置草稿/发布/历史版本回滚，以及风险告警指派/核查/解决两条 Chromium 闭环；同时修复写操作成功提示在详情刷新时被清空的问题。隔离种子只使用合成候选、证据、告警、管理员和 TOTP；Admin 51 项测试、完整 16 项 Chromium 旅程与统一本地门禁已通过，远端 CI 待本轮推送后复核。Web 邮箱/刷新异常组合、视觉、移动端、可访问性、目标环境和生产验收仍未关闭。
