...
> 2026-08-09 WP3 第 1 次迭代已建立可重复的 Playwright 隔离环境，使用临时 SQLite、合成 Web/Admin 账号和真实 FastAPI/Vite 服务，覆盖 Web 注册/登录/退出及 Admin 首次 TOTP 绑定/二次登录；同时修复真实浏览器中 TOTP 手工密钥输入值未同步的问题。查询、贡献、案件处置、视觉、移动端、目标环境和生产验收仍未关闭。
