...
- 最近迭代：[WP3 第 7 次开发迭代：Web 邮箱、刷新令牌与移动端键盘门禁](./2026-08-09-wp3-iteration-7.md)
- 前次迭代：[WP3 第 6 次开发迭代：系统配置与风险告警浏览器闭环](./2026-08-09-wp3-iteration-6.md)
- 前序迭代：[WP3 第 5 次开发迭代：Admin 用户治理与角色双人审批闭环](./2026-08-09-wp3-iteration-5.md)
...
> 2026-08-09 WP3 第 7 次迭代新增真实 FastAPI/Vite 服务下的邮箱待验证/一次性验证/重放拒绝、刷新令牌轮换/旧令牌重放/令牌族撤销，以及 390 × 844 移动视口无横向溢出和键盘返回登录三条 Chromium 门禁。隔离种子使用合成用户、凭证摘要和刷新会话，未放宽生产登录限流；完整 19 项 Chromium 旅程与统一本地门禁已通过；Admin 候选审核用例已支持复用已变更数据库状态的 CI 重试，并以同一 Playwright 服务连续执行 2 次验证。远端 GitHub Actions `31301934873`（功能提交）与 `31302927800`（重试稳定性提交）的 API、Frontend、Desktop、E2E 和 Container Images 五个作业均全部通过。视觉快照、更多页面移动端/可访问性、目标环境和生产验收仍未关闭。
...
| MVP 验收完成度 | 约 87% | 统一本地门禁历史轮次和 1440 px Web 静态视觉复核通过，管理端已具备用户处置、配置治理和普通角色双人审批，Web/Admin 19 项核心浏览器旅程通过；视觉回归 E2E、实机桌面 E2E、规模压测、安全扫描、恢复演练、真实 SMTP 服务商/发件域名和生产密钥验收未完成 |
