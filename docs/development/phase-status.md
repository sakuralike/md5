...
- 更新日期：2026-08-03
- 当前里程碑：M4 社区信任与管理闭环
- 最近迭代：[M4 第 7 次开发迭代](./2026-08-03-m4-iteration-7.md)
...
| 维度 | 工程估算 | 说明 |
|---|---:|---|
| 功能代码完成度 | 约 91% | M1、M2、M3 主流程以及 M4 候选审核、举报/申诉、积分信誉、奖励补偿、失败激增、关联降权、告警 SLA/指派/通知闭环已形成；跨候选图谱、用户处置、案件编排和 M5 发布工程仍有明显缺口 |
| MVP 验收完成度 | 约 78% | 自动化门禁、M4 审核/案件/信誉/补偿/告警/关联/SLA 专项测试稳定，但浏览器 E2E、实机桌面 E2E、规模压测、生产通知和生产密钥验收未完成 |
| 生产上线准备度 | 约 47% | 容器、MySQL/SQLite 迁移、CI、不可变奖励校正、告警 SLA 与通知 Outbox 基线已建立；生产通知提供商、监控平台、安全扫描、恢复演练、合规和灰度发布尚未闭环 |
...
| M4 社区信任与管理闭环 | 约 92% | 候选审核、举报/申诉、积分投影、不可变信誉事件、奖励补偿、失败激增、候选内关联降权和告警 SLA/指派/通知首版完成；跨候选图谱、用户处置、案件编排和生产通知待实现 |
...
- 统一门禁通过：API 默认环境 59 项通过/1 项真实 Redis 条件测试跳过，真实 Redis 环境 60 项全部通过且覆盖率 90.67%；共享契约 2 项、Web 10 项、Admin 18 项、Desktop 11 项通过。SQLite/MySQL 8.4 全量迁移往返和 Docker Compose 空环境冒烟通过，合成注册/登录耗时约 1.0 分钟；托管 CI 结果在 PR 创建后补充。
...
| 异常检测和告警 | 三个垂直切片完成 | `risk-alert-v1`、`correlation-v1` 与 `risk-alert-sla-v1` 已实现失败激增、候选内关联降权、MFA 值班指派、负责人/超时队列、事务通知 Outbox、Worker 有限重试和聚合最小披露；生产通知提供商、排班升级链、指标和跨候选分析待实现 |
