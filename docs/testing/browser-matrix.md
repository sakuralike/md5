...
- 更新日期：2026-08-09
- 当前自动化基线：Playwright Chromium，Web/Admin 共 30 项旅程
- 结论边界：本地 Windows Chromium 与 GitHub Actions Linux Chromium 已验证；其他浏览器、操作系统和真实设备均不得据此标记通过
...
| GitHub Actions | `ubuntu-latest` | Playwright Chromium | 测试配置固定视口 | 30 项核心 E2E，失败制品上传 | 已通过，运行 `31310591798` |
...
| 跨引擎 | Linux CI | Firefox Stable | 1440 × 900、390 × 844 | Playwright 核心旅程 | 未接入 |
| Apple Web | macOS | Safari Stable | 桌面与响应式 | 人工功能、视觉、VoiceOver | 未验证 |
| WebKit 预检 | Linux/macOS | Playwright WebKit | 1440 × 900、390 × 844 | 核心旅程与布局 | 未接入 |
...
- Ubuntu CI 的 API、Frontend、Desktop、Container Images 和 Chromium E2E 作业。
- Chromium 核心旅程、页面级无横向溢出、关键区域边界和 Axe 严重/关键违规门禁。
...
1. 在 CI 增加 Firefox 核心旅程试运行，不立即阻断全部分支，先治理兼容差异。
2. 建立 Edge/Windows 11 目标环境执行记录，完成 NVDA 清单。
3. 增加 WebKit 预检并安排 macOS Safari + VoiceOver 人工验收。
4. 完成 iOS Safari、Android Chrome 的真实设备抽样。
5. 将通过记录、缺陷和风险接受汇总到 WP3 出口报告，再转入 WP4 目标环境安全、恢复、性能与可观测性准入。
