...
| DESK-09～12 | `modules/desktop_updates`、`apps/desktop-windows` | `apps/api/tests/test_m3_desktop_updates.py`、`apps/desktop-windows.tests/DesktopUpdateClientTests.cs`：版本/目标选择、声明大小与摘要、原始制品上传、发布/撤回、生产签名元数据门禁、匿名检查、下载完整性、客户端查询参数和清单解析 | M3 后端发布通道与显式升级入口完成；正式 Authenticode 流水线、分批发布和静默安装留后续 |
