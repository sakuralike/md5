...
| VERIFY-01～07 | `modules/verification`、`modules/moderation`、`modules/reputation` | `apps/api/tests/test_m2_verification.py`、`apps/api/tests/test_m4_candidate_moderation.py`：单账号唯一有效反馈、不可变历史、关联键去重、双成功验证、三失败隔离、verified 降级、自动/人工状态事件、规则版本、积分幂等和 rejected 普通反馈锁定 | M2 自动状态、M3 桌面签名回执和 M4 人工审核首版完成；关联账号/短时异常与告警待实现 |
| 积分与信誉、用户中心 | `modules/reputation`、`modules/verification`、`apps/web` | `apps/api/tests/test_m4_reputation_center.py`、`apps/api/tests/test_m2_verification.py`、`apps/web/src/services/reputation.test.ts`：初始 50 分、积分状态投影、首次有效贡献/验证信誉结算、引用幂等、0～100 边界、本人私有读取和 Web 鉴权加载 | M4 第 3 次迭代首版完成；撤销补偿、管理员用户处置和动态信誉权重待实现 |
...
| 管理端需求 | `modules/admin`、`modules/moderation`、`modules/trust_cases`、`modules/desktop_updates`、`apps/admin` | `apps/api/tests/test_auth.py`、`apps/api/tests/test_m1_security_gates.py`、`apps/api/tests/test_m4_candidate_moderation.py`、`apps/api/tests/test_m4_trust_cases.py`、`apps/admin/src/services/candidateModeration.test.ts`、`apps/admin/src/services/trustCases.test.ts`、`apps/admin/src/services/desktopReleases.test.ts`：RBAC、普通用户拒绝、TOTP/MFA、最小披露审核详情、候选/案件原因与目标匹配、人工处置幂等/审计、发布元数据与原始制品上传 | M1 安全入口、M3 桌面发布和 M4 候选审核/案件队列首版完成；危险动作二次确认、用户处置和对象级批量接口待实现 |
| 前端认证传输 | Web/Admin API Client | `apps/web/src/services/api.test.ts`、`apps/admin/src/services/api.test.ts` | M1 基础完成 |
| 敏感数据日志保护 | `core/logging`、`modules/moderation`、`modules/trust_cases` | `apps/api/tests/test_security.py`、`apps/api/tests/test_m4_candidate_moderation.py`、`apps/api/tests/test_m4_trust_cases.py`：嵌套字段递归脱敏；候选审核不返回秘密材料，人工处置审计不复制候选秘密、用户说明或管理员自由文本 | M2 日志基础与 M4 审核/案件审计最小披露完成 |
