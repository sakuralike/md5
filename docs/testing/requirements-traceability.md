...
| 关键写幂等 | `core/idempotency`、`modules/archives`、`modules/verification`、`modules/moderation` | `apps/api/tests/test_m1_security_gates.py`、`apps/api/tests/test_m2_archive_core.py`、`apps/api/tests/test_m2_verification.py`、`apps/api/tests/test_m4_candidate_moderation.py`：缓存响应、请求冲突、密码重置、贡献、反馈与人工状态处置 | M1/M2 基础完成；M4 人工处置重复请求不产生重复状态事件 |
...
| VERIFY-01～07 | `modules/verification`、`modules/moderation` | `apps/api/tests/test_m2_verification.py`、`apps/api/tests/test_m4_candidate_moderation.py`：单账号唯一有效反馈、不可变历史、关联键去重、双成功验证、三失败隔离、verified 降级、自动/人工状态事件、规则版本、积分幂等和 rejected 普通反馈锁定 | M2 自动状态、M3 桌面签名回执和 M4 人工审核首版完成；关联账号/短时异常与告警待实现 |
...
| 管理端需求 | `modules/admin`、`modules/moderation`、`modules/desktop_updates`、`apps/admin` | `apps/api/tests/test_auth.py`、`apps/api/tests/test_m1_security_gates.py`、`apps/api/tests/test_m4_candidate_moderation.py`、`apps/admin/src/services/candidateModeration.test.ts`、`apps/admin/src/services/desktopReleases.test.ts`：RBAC、普通用户拒绝、TOTP/MFA、最小披露审核详情、原因与目标匹配、人工处置幂等/审计、发布元数据与原始制品上传 | M1 安全入口、M3 桌面发布和 M4 候选审核首版完成；危险动作二次确认、举报/用户处置和对象级批量接口待实现 |
| 前端认证传输 | Web/Admin API Client | `apps/web/src/services/api.test.ts`、`apps/admin/src/services/api.test.ts` | M1 基础完成 |
| 敏感数据日志保护 | `core/logging`、`modules/moderation` | `apps/api/tests/test_security.py`、`apps/api/tests/test_m4_candidate_moderation.py`：嵌套字段递归脱敏；审核列表/详情不返回秘密材料，人工处置审计不复制自由文本原因或密码字段 | M2 日志基础与 M4 审核最小披露完成 |
