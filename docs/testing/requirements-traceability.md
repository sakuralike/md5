...
| SEO-08 内容级 SEO 与 SSR 规划 | `项目文档/实施计划/SEO内容级字段与SSR实施计划.md`；`apps/api/src/password_detective/modules/community/seo_projection.py`；社区实体、状态与权限设计输入 | 阶段 7-A 后端社区定向测试；阶段 7-B `test_community_post_seo.py`、`test_community_group_seo.py`、迁移头检查、API 合同类型检查与 Web 回归测试；后续仍需动态 sitemap、SSR/预渲染专项验收 | 阶段 7-A 已实现受控 `seo` 投影与公开资格矩阵；阶段 7-B 已实现帖子和群组 SEO 字段、迁移、授权 PATCH、幂等、版本冲突、安全校验及 Web 编辑入口；`eligible` 与 `indexable` 仍分离，当前动态内容保持 `indexable=false`。用户字段、动态 sitemap、SSR/预渲染和百度推送仍待后续实施 |
