...
| 管理端需求 | `modules/admin`、`modules/moderation`、`modules/trust_cases`、`modules/desktop_updates`、`apps/admin` | `apps/api/tests/test_auth.py`、`apps/api/tests/test_m1_security_gates.py`、`apps/api/tests/test_m4_candidate_moderation.py`、`apps/api/tests/test_m4_reward_compensation.py`、`apps/api/tests/test_m4_trust_cases.py`、`apps/admin/src/services/candidateModeration.test.ts`、`apps/admin/src/services/trustCases.test.ts`、`apps/admin/src/services/desktopReleases.test.ts`：RBAC、普通用户拒绝、TOTP/MFA、最小披露审核详情、候选/案件原因与目标匹配、人工处置幂等/审计、奖励校正汇总/时间线、发布元数据与原始制品上传 | M1 安全入口、M3 桌面发布和 M4 候选审核/案件队列/奖励校正展示完成；危险动作二次确认、用户处置和对象级批量接口待实现 |
