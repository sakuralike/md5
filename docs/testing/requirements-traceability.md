...
| SEO-08 内容级 SEO 与 SSR 规划 | `项目文档/实施计划/SEO内容级字段与SSR实施计划.md`；社区实体、状态与权限设计输入 | 规划文档固定帖子/群组/用户/板块资格矩阵、字段校验、权限审计、动态 sitemap、SSR/预渲染与百度推送边界 | 已完成规划；运行时字段、动态 sitemap、SSR/预渲染和百度推送待后续确认与实施 |
