...
- 机器校验：`scripts/verify_monitoring_config.py`、`scripts/verify_worker_backlog_evidence.py`。

监控配置只读取脱敏、低基数指标；不得把请求号、用户标识、邮箱、完整指纹、令牌或密码放入 Prometheus 标签、Grafana变量或告警表达式。生产环境还需要独立凭据、网络白名单、Alertmanager通知和目标环境验收。
