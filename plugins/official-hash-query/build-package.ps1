...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $Root ".local/official-hash-query/official-hash-query.pdpkg"
} else {
    $OutputPath = [System.IO.Path]::GetFullPath($OutputPath)
}
$PublishDirectory = Join-Path $Root ".local/official-hash-query/publish-1.0.0"
New-Item -ItemType Directory -Force -Path $PublishDirectory, (Split-Path -Parent $OutputPath) | Out-Null

& dotnet publish (Join-Path $Root "plugins/official-hash-query/PasswordDetective.OfficialHashQuery.csproj") `
...
if ($LASTEXITCODE -ne 0) { throw "Official hash query publish failed." }

& dotnet run --project (Join-Path $Root "tests/pdpp-package-builder/PdppPackageBuilder.csproj") --configuration Release -- `
    (Join-Path $PublishDirectory "password-detective-official-hash-query.exe") $OutputPath `
    (Join-Path $PSScriptRoot "manifest.template.json") `
    (Join-Path $PSScriptRoot "schemas/query.schema.json") `
    (Join-Path $PSScriptRoot "sbom.cdx.json") $PrivateKey
if ($LASTEXITCODE -ne 0) { throw "Official hash query package build failed." }

Write-Host "Official hash query package created: $OutputPath"
