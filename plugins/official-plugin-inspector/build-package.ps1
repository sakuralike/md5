...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $Root ".local/official-plugin-inspector/official-plugin-inspector.pdpkg"
} else {
    $OutputPath = [System.IO.Path]::GetFullPath($OutputPath)
}
$PublishDirectory = Join-Path $Root ".local/official-plugin-inspector/publish-1.0.0"
New-Item -ItemType Directory -Force -Path $PublishDirectory, (Split-Path -Parent $OutputPath) | Out-Null

& dotnet publish (Join-Path $Root "plugins/official-plugin-inspector/PasswordDetective.OfficialPluginInspector.csproj") `
...
if ($LASTEXITCODE -ne 0) { throw "Official plugin inspector publish failed." }

& dotnet run --project (Join-Path $Root "tests/pdpp-package-builder/PdppPackageBuilder.csproj") --configuration Release -- `
    (Join-Path $PublishDirectory "password-detective-official-plugin-inspector.exe") $OutputPath `
...
if ($LASTEXITCODE -ne 0) { throw "Official plugin inspector package build failed." }

Write-Host "Official plugin inspector package created: $OutputPath"
