...
| 公开社区帖子/群组页 | 首阶段使用页面类型标题；帖子和群组进入内容级 SEO 独立切片 | 使用固定页面简介，不读取私密正文 | 首阶段允许公开路由元标签，但不加入未验证的动态地址 sitemap |
| 用户资料页 | 不纳入 SEO；资料和关系页面始终保持 `noindex, nofollow` | 不生成 SEO 字段、不进入 sitemap、不提供 SEO 编辑入口 | 用户资料属于隐私域 |
...
- 允许：`/`、`/hash/...` 的公开详情、`/community`、公开群组/帖子详情的稳定地址。
...
- 对需要数据库分页才能发现的动态帖子/群组地址，首阶段不直接全量加入 sitemap；第二阶段完成公开内容查询、状态过滤、更新时间和分页后再纳入。
...
- 动态帖子、群组详情的“首个 HTML 响应即含准确内容标题/摘要”需要 SSR、预渲染或边缘 HTML 注入；用户资料不进入 SEO 流程；首阶段不宣称已经达到 Zibll/PHP 服务端渲染的同等抓取效果。
...
- [x] 将本计划中已确认的首阶段 SEO 需求写入当前 v3.0 规格的系统配置、公开站点和验收章节。
- [x] 明确“直接保存、无发布/版本/差异/回滚”与 SEO 设置一致。
- [x] 增加 Vue SPA 动态元标签与 SSR 边界说明，避免将客户端更新误报为服务器端 SEO 完成。
- [x] 在变更记录中记录规格、API、数据模型和验收标准的同步位置。

### Task 2：后端 SEO 配置模型与当前配置服务

**文件：**

- 修改：`apps/api/src/password_detective/modules/admin/setting_schemas.py`
- 新增：`apps/api/src/password_detective/modules/admin/seo_settings.py`
...
- 新增：`apps/api/tests/test_seo_settings.py`
- 修改：`apps/api/tests/test_site_discovery.py`

- [x] 定义 `SeoSettings`、管理响应和公共响应模型，落实关键词、URL、标题和描述校验。
- [x] 以 `seo_settings` 为固定当前配置键实现读写；保存使用已有管理员权限、幂等键、审计和事务约束。
- [x] 将数据库缺省、旧配置缺省和异常配置统一归一到安全默认值。
- [x] 扩展 `/site/config` 只返回已清洗的 `PublicSeoConfig`，不得暴露数据库键、操作者或审计字段。
- [x] 编写测试证明重复幂等请求不会重复写入、非法 URL/控制字符/超长字段被拒绝、关闭索引时公共配置仍不泄露私密信息。

**2026-08-18 实施回填：**

- 已完成管理端 `GET/PUT /api/v1/admin/settings/seo`，沿用管理员 MFA/角色门禁、幂等键、事务提交和最小审计摘要；不新增版本、差异、发布或回滚模型。
- 已完成 `seo_settings` 单键持久化、异常配置安全回退、关键词去重和安全 URL/纯文本校验；Staging 默认 `indexing_enabled=false`。
- 已扩展 `/api/v1/site/config` 的清洗公开 SEO 投影，并从公开响应中排除 `sitemap_enabled`、SMTP 密码和其他管理字段。
- 本回填仅表示后端与 API 合同完成；`robots.txt`、`sitemap.xml`、Admin 工作台、Web 运行时元标签和 Staging 真实浏览器验收仍未完成。

**2026-08-18 Staging 部署回填：**

- 运行镜像：`password-detective-api:b91c75f20a44034c81113de609f28e578721a9f0`；远端部署提交与本地提交一致。
- Staging 采用 API HA Compose 覆盖层清除 API 宿主机端口冲突，保持 `api-proxy` 对外提供 `127.0.0.1:8000`，API 运行 2 个实例、Worker 3 个实例。
- 远端验收：API 两个实例、`api-proxy`、Worker 三实例、Scheduler 均为运行状态；两个 API 实例和 `api-proxy` 健康检查通过；`GET /api/v1/health/ready` 返回 HTTP 200；`GET /api/v1/site/config` 返回安全默认 SEO 投影；未认证访问 `GET /api/v1/admin/settings/seo` 返回 HTTP 401。
- 本回填不替代根路径 `robots.txt`/`sitemap.xml`、Admin 浏览器工作台、Web 元标签和完整 Staging 浏览器矩阵验收。
...
- [x] 实现公开路由注册表，所有可收录路由必须显式标记 `seoIndexable=true`，不使用“未标记即公开”。
- [x] 实现 `robots.txt` 响应，分别覆盖索引关闭、站点地图关闭和正常公开索引三种状态。
- [x] 实现合法 XML `sitemap.xml` 响应，进行 URL 规范化、查询/哈希移除、实体转义和公开状态过滤。
- [x] 在反向代理中将网站根路径映射到 API/应用对应端点，保证搜索引擎不需要知道内部 `/api/v1` 前缀。
- [x] 测试不得出现 `/admin`、`/login`、`/community/messages`、搜索查询参数、会话参数或任何需认证 URL。

**2026-08-18 实施回填：**

- 已新增 `apps/api/src/password_detective/modules/site/seo.py`：以显式的 `PUBLIC_ROUTE_REGISTRY`（`seo_indexable=true`）仅登记 `/` 与 `/community`，未登记路径不得写入站点地图。
- 已新增 `GET /api/v1/site/robots.txt` 与 `GET /api/v1/site/sitemap.xml`：默认全站禁止抓取；仅当 SEO、收录和站点地图均开启时输出白名单 URL。`PUBLIC_ORIGIN` 为受校验的部署配置，不信任请求 Host。
- 已在 `infra/nginx/spa.conf` 映射根路径 `/robots.txt` 与 `/sitemap.xml` 到内部 API 端点；新 Nginx 配置已通过 `nginx -t`。
- 已新增 `apps/api/tests/test_site_seo_files.py` 覆盖默认关闭、公开白名单、恶意 Host 不影响 Sitemap Origin、索引/站点地图关闭和 XML 输出；公开动态内容和浏览器元标签仍由后续任务完成。
- 本地验证通过：SEO 文件定向 pytest 4 项、API 全量 pytest、定向 Ruff、Web build/typecheck、Web ESLint 与 Web Vitest 45 个文件/78 项；部署镜像内 Web 构建同步通过。
- SSH 远端分支已推进到 `bec0271417c5912269dbe566ece33052e1292957`（Task 3 主提交 `89acf923f6e21f84ff8fc9ef510e660723ac26d5`，随后修复 Web 内置公开配置缺少安全 SEO 默认值）。
- Staging 已部署 `bec0271417c5912269dbe566ece33052e1292957`：API 2 实例健康、Worker 3 实例运行、Scheduler 与 API Proxy 正常；API 就绪探针、根路径与内部 `robots.txt`/`sitemap.xml` 均返回 HTTP 200。默认索引关闭状态下 robots 精确禁止全站抓取，sitemap 为合法空 `urlset`，Nginx 配置检查通过。
...
- 新增：`apps/admin/src/lib/seoSettingsForm.ts`
- 新增：`apps/admin/src/pages/SystemSettingsSeo.test.ts`
- 新增：`apps/admin/src/components/AdminSettingsNavigation.test.ts`

- [x] 增加管理端和公共端 SEO 类型，保持 snake_case API 字段与 TypeScript 类型一致。
- [x] 新增 SEO 表单的加载、保存、重置、错误和成功提示状态；保存时生成幂等键，成功后更新基线。
- [x] 实现关键词逗号输入和本地预览，不把预览状态发送为额外版本对象。
- [x] 为 `#seo-settings` 增加桌面固定导航和移动抽屉入口，使用 `scroll-mt-28`（112px）满足固定顶栏偏移不少于 104px。
- [x] 测试证明刷新请求读取当前配置、表单克隆隔离编辑、重置可恢复最后基线，页面不存在发布/回滚/版本历史/差异预览控件。

**Task 4 实施与发布证据（2026-08-18）：**

- Admin 工作台已接入 `GET/PUT /api/v1/admin/settings/seo`；保存使用客户端幂等键，成功后直接更新当前基线，重置仅恢复最近一次加载或保存的值。
- 关键词输入支持半角和全角逗号，保存前执行 trim 与去重；标题、描述、连接符、默认图片和索引/Open Graph/站点地图开关均有对应控件；搜索结果为本地预览，不形成快照或版本对象。
- 桌面固定导航与移动抽屉均新增 `#seo-settings` 入口，并移除“版本历史”入口；用户等级与权益仍只保留在用户审批工作台。
- 本地验证：SEO 表单/请求/页面/导航定向测试 6 项通过；Admin 全量 Vitest 46 个文件、93 项测试通过；Admin `typecheck`、ESLint 和生产构建通过。
- 远端 SSH 分支已推进至 Admin 工作台提交 `21cff36d6ee9799771fade49035a4716392425a1`；Staging 同步构建并运行 API、Web、Admin 镜像，部署标记与提交/树/归档校验一致。API 2 实例、Worker 3 实例、Scheduler、API Proxy、Web、Admin 均运行，API 就绪、Web 根路径、Admin 根路径、robots、sitemap 返回 HTTP 200，robots 默认全站禁止抓取，sitemap 为合法空 `urlset`，Nginx 配置检查通过。
...
- [x] 建立路由元数据白名单，显式标注公开、访客和私密页面。
- [x] 实现标题、描述、关键词、canonical、robots、Open Graph 生成和旧标签清理。
- [x] 将查询参数、哈希片段、会话信息和私密数据排除出 canonical、sitemap 和元标签。
- [x] 在 API 未响应时保持安全的默认标题与 `noindex` 策略，不能因为加载失败而把所有页面误标为可收录。
- [x] 测试路由切换、重复标签、认证路由、索引关闭和默认配置回退。

**2026-08-18 实施回填：**

- Web 运行时代码位于 `apps/web/src/lib/seo.ts`、`App.vue`、`router/index.ts`、`services/site.ts` 与 `index.html`；仅 `/`、`/community` 允许索引，认证、访客及未列入白名单的路由默认 `noindex, nofollow`。
- 已实现 title、description、keywords、canonical、robots、Open Graph 的集中生成与旧标签清理；canonical 不含查询参数或哈希，公开配置加载失败、索引关闭或 Origin 不合法时均回退安全默认。
- 本地验证：SEO 单元测试 4 项通过；Web 全量 Vitest 46 个文件、82 项测试，typecheck、ESLint 与生产构建通过。
- SSH 分支与 Staging Web 运行时已同步提交 `3861663aa9401c455b964dc3601ae454b37cf524`；运行镜像为 `password-detective-web:3861663aa9401c455b964dc3601ae454b37cf524`。
...
- [x] 后端执行：`cd apps/api; .venv/Scripts/python -m pytest tests/test_seo_settings.py tests/test_site_discovery.py tests/test_site_seo_files.py -q`。
- [x] Admin 执行：`pnpm --filter @password-detective/admin test`、`pnpm --filter @password-detective/admin lint`、`pnpm --filter @password-detective/admin typecheck`。
- [x] Web 执行：`pnpm --filter @password-detective/web test`、`pnpm --filter @password-detective/web lint`、`pnpm --filter @password-detective/web typecheck`。
- [x] 统一执行：`pnpm typecheck`、`pnpm lint`、`pnpm test`，并记录跳过的外部依赖检查。
- [x] Chromium 验收管理端保存/重置和 Web 端路由元标签；Firefox/WebKit 复跑关键元标签与无控制台错误旅程。
- [x] 通过真实 HTTP 请求检查 `/robots.txt` 的 `Content-Type`、索引开关和 Sitemap 指令，以及 `/sitemap.xml` 的 XML 合法性和公开 URL 白名单。
- [x] 完成 Staging 部署后记录实际部署修订、Web/API HTTP 状态、页面源代码与运行后 head 的差异；不把本地构建或静态归档作为 Staging 证据。

**2026-08-18 实施回填：**

- 后端定向 pytest（`test_seo_settings.py`、`test_site_discovery.py`、`test_site_seo_files.py`）通过；Admin 全量 Vitest 46 个文件、93 项测试通过；Web 全量 Vitest 46 个文件、82 项测试通过；全仓 `pnpm typecheck`、`pnpm lint`、`pnpm test` 与 `pnpm build` 通过。
- 新增 `tests/e2e/admin-seo-settings.spec.ts` 与 `tests/e2e/web-seo-journeys.spec.ts`。Admin 保存/重置与 Web 首页、社区、登录路由旅程在 Chromium、Firefox、WebKit 共 9 项通过，页面错误和浏览器控制台错误均为空。
- SSH 已推送验收提交 `6fbc249377718fdaea3876b7d13de9cef3960f9f`；Staging 已同步该提交的 QA 归档。最终运行时保留已验证的 Web `3861663aa9401c455b964dc3601ae454b37cf524`、Admin/API `21cff36d6ee9799771fade49035a4716392425a1` 镜像：API 2 实例、Worker 3 实例、Scheduler、API Proxy、Web、Admin 均运行；就绪探针、网站/API 的 robots 与 sitemap 均为 HTTP 200，robots 为 `text/plain; charset=utf-8`、sitemap 为 `application/xml`，Nginx 配置检查通过。
- 本任务未新增运行时代码镜像；外部生产数据库/Redis、真实 SMTP 投递及 SSR/预渲染收录不属于本轮验收，未作完成性声明。
...
- [x] 已完成社区帖子/群组的 SEO 字段、公开投影、授权编辑入口和浏览器运行时元数据兼容层；用户资料页不新增 SEO 字段。
- [ ] 动态 sitemap 与 SSR/预渲染仍需独立设计和验收，通过前不得开放动态内容索引。
- [ ] 只允许内容所有者或授权管理员编辑 SEO 字段；私密、隔离、删除和未审核内容不得进入 sitemap。
- [ ] 另行设计搜索引擎推送凭据的密钥托管、最小权限、轮换、审计和撤销流程，再决定是否接入百度普通/快速收录。
#### 2026-08-18 规划回填

本轮已完成 [`SEO内容级字段与SSR实施计划.md`](./SEO内容级字段与SSR实施计划.md)，固定了 `CommunityPost`、`CommunityGroup` 与板块候选实体的公开资格、SEO 字段、编辑权限、API 投影、动态 sitemap、SSR/预渲染及百度推送边界；用户资料属于隐私域，不纳入 SEO。

#### 2026-08-18 阶段 7-A 实现回填

已完成阶段 7-A 的公开资格与 SEO 投影：帖子和群组详情返回受控 `seo` 字段；服务端统一输出 `eligible`、`indexable`、标题、清洗摘要、站内 canonical 路径和空 OG 图片。用户资料响应不返回 SEO 字段，用户资料、关系和账户路径继续保持私密 `noindex`。当前 Vue SPA 仍固定 `indexable=false`，不开放动态 sitemap、不新增 SEO 数据库字段/迁移，也不宣称 SSR/预渲染收录能力。阶段 7-B 的帖子/群组可编辑 SEO 字段、迁移、权限与审计仍按独立切片推进。

#### 2026-08-19 内容详情 Web 运行时接入回填

帖子和群组详情页已将服务端受控 `seo` 投影接入全局 Web head 管理器；只有 `eligible=true` 的投影会生成内容标题、摘要、关键词、站内 canonical 和 Open Graph。动态内容继续输出 `noindex, nofollow`，不进入 sitemap，也不宣称 SSR/预渲染收录。审批/私密群组、无资格内容、加载失败、过期路由请求和用户资料均保持安全空回退。
...
**本轮结论：** SEO 首阶段 Task 1～Task 6 已完成：规格治理、当前配置后端、公开配置投影、根路径 `robots.txt`/`sitemap.xml`、Admin SEO 直接保存工作台、Web 运行时元标签、统一质量门禁和三浏览器矩阵均已有证据。Task 6 的 QA 归档修订为 `6fbc249377718fdaea3876b7d13de9cef3960f9f`；其未引入新的运行时代码镜像，Staging 继续运行已验证的 Web `3861663aa9401c455b964dc3601ae454b37cf524` 与 Admin/API `21cff36d6ee9799771fade49035a4716392425a1`。首阶段仍不宣称 SSR/预渲染收录保证，内容级 SEO、SSR/预渲染与百度推送留待独立评审。
