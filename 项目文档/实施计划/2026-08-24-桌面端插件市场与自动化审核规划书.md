...
- 在主进程中加载第三方 .NET Assembly、原生 DLL、COM、浏览器扩展或第三方 WPF 控件。
- 允许插件向主进程注入未经宿主清洗的 HTML、JavaScript、XAML 或原生窗口句柄；需要 WPF 界面的插件必须在自身 AppContainer 进程内创建自有窗口，并声明 `ui:window`。
...
v1 插件不能注入主进程的自定义 WPF 控件。默认插件在清单中声明命令及 JSON Schema 输入，宿主使用原生控件渲染；声明 `ui:window` 并通过 `host/ui/window/open` 获得授权的插件，可以在自有 AppContainer 进程中使用独立 WPF SDK 创建窗口：

- 文本、数字、布尔、枚举、文件选择和目录选择。
- 进度、表格、键值列表、纯文本、受限 Markdown 和文件导出结果。
- 命令图标使用宿主内置图标名，不接受插件 SVG/XAML。
- 外部链接必须由宿主校验并在系统浏览器中打开。
- 插件不能控制宿主顶层窗口、宿主剪贴板或宿主通知；隔离窗口的标题、尺寸和模态属性必须经过 `ui:window` 权限校验。
...
**对象存储切片（2026-08-26）：** 新增可选 S3 兼容插件存储后端，保留 filesystem 默认值；上传、审核读取、发布、下载、撤销和审核证据写入均支持 S3 对象与本地缓存一致性校验。Staging 已部署私有 MinIO 与对象卷，API/Worker 跨容器写入/读取合同通过，迁移为 `20260826_0060 (head)`；部署修订 `6c0d26c6f5da105e3665e6dd32b927dd7432cab7`，源码树 `6961886d5c000f087606038175a96fc866f7b890`，归档 SHA-256 `40c85dfce9f0c675aed6fad4cbfe2c1d009048c5b081fec71bc3ae75487a453b`，回滚目录 `/opt/password-detective-staging.previous-20260826T113401Z-6c0d26c6`。CDN、真实 VM/mTLS、长时压力、UAT 和 Production 仍未验证。
