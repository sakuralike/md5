...
> **2026-08-11 发布治理决策（当前有效）**：固定 4 小时 Staging 稳定性验证不再是项目必需门禁；以部署 smoke、拓扑/容量预检、短时混合回归、单 Worker 恢复、资源阈值、备份恢复、密钥轮换、回滚和必要的 HA 证据作为当前验收依据。既有长时会话脚本和证据格式保留为可选诊断，不计入未完成项。
>
> **2026-08-11 发布治理决策（其他内容）**：项目不要求任何人员签字确认，Staging/发布结论由 `automated-evidence-gate` 依据完整、脱敏且校验通过的证据包自动输出。桌面端不要求 Authenticode 或签名证书；桌面制品必须由服务端重算文件大小和 SHA-256，并附带已确认的合法分发声明。该声明用于审计追溯，不构成外部法律意见。远端同步只使用 GitHub Git Data API 的等效提交链路，不使用 HTTP/1.1 `git push`。产品内 N2 角色变更的双人审批不受本决策影响。
...
- 文件型 Docker Secrets 目标部署、加密备份异地保管、定期轮换和应急吊销流程；外部 KMS 暂不作为必选项。
...
### 9.22 WP4 第 24 次迭代完成：正式 Staging 长时会话后台控制（2026-08-11）

- 新增可脱离 SSH 持续运行的正式会话 supervisor，默认固化 4 小时、双 API、3 Worker、固定探针窗口、资源采样和单 Worker 恢复事件。
- 独占状态锁、每次唯一输出目录、PID 启动标记和原子状态更新阻止并发执行、目录覆盖与 PID 复用误判。
- 状态和日志不持久化环境变量或秘密；完成状态绑定验证报告 SHA-256，并显式区分 runner 完成与 `eligible_for_target_execution` 资格。
- 新增 `pnpm staging:formal-session`、7 项单元测试及统一门禁接入。
- 目标 Staging 正式会话已于 2026-08-11 19:31:10（Asia/Shanghai）启动，运行标识为 `20260811T113110Z-eb710ed0`；等待真实 4 小时结果，通过后依次执行 MySQL/Redis HA、证据封存和自动发布证据决策。
...
### 9.23 WP4 第 25 次迭代完成项（2026-08-11）

- 新增 HA 执行前置门禁，将 MySQL 切换严格绑定到已完成、通过且摘要一致的正式 4 小时稳定性证据。
- Redis HA 额外要求通过严格验证的 MySQL `target-execution` 报告，禁止使用合同夹具或并行执行绕过依赖顺序。
- 新增 `pnpm staging:ha-preflight`、7 项测试、统一门禁和 CI 作业；正式会话运行期间目标 Staging 预检应安全输出 `blocked`。
- 下一轮在正式会话完成后先复核证据和 MySQL 预检，再执行真实托管 MySQL HA 切换；MySQL 证据通过后才进入 Redis。

- 目标 Staging 已于 2026-08-11 20:01:56（Asia/Shanghai）部署 HA 门禁并执行 MySQL 预检；正式会话仍为 `running`，门禁按设计输出 `blocked`，未触发故障动作。

### 9.24 WP4 第 26 次迭代完成项（2026-08-11）

- 新增 MySQL/Redis HA 目标能力证据合同和严格校验器，要求跨故障域、副本、自动切换、稳定应用地址、监控、备份/持久化及 RTO/RPO 均满足 profile。
- MySQL `managed-ha` 只能由真实托管服务能力证明；Redis 只接受托管 HA 或至少三个 Sentinel 的自管拓扑。
- HA 执行门禁新增 `--capability-report`，缺失、无效、单节点或 `contract-fixture` 均不得放行。
- 新增双依赖合同、17 项组合测试、统一门禁和 CI 证据上传；下一轮等待正式稳定性证据并获取真实平台 `target-observation`。

### 9.27 WP4 第 27 次迭代完成：HA 能力证据封存绑定与当前完成度校正（2026-08-11）

- `target-execution` 封存包新增强制 MySQL/Redis `target-observation` 能力报告，能力摘要、文件 SHA-256 和主 checksum 一起进入确定性归档及 `evidence_set_sha256`。
- 缺失能力报告、合同夹具冒充真实观察或敏感字段均阻断 `ready-for-release`；`contract-fixture` 保持 `not-run/pending-evidence`。
- 当前基线校正为核心功能约 97%、本地工程约 99%、综合研发约 89%、生产上线就绪约 76%；人员签字和桌面签名证书不再是发布阻塞。
- **下一轮开发入口：** 等待正式 Staging 会话完成并复核；恢复当前提交远端 CI；取得真实托管 MySQL/Redis 能力报告后依次执行 HA、封存并由自动证据门禁输出结论。


## 2026-08-11 WP4 第 28 轮执行结果：文件型生产秘密方案

- 产品决策：不接入外部 KMS，采用文件型 Docker Secrets、项目现有 AES-GCM 密钥环、独立生产密钥、权限控制、认证加密备份和定期轮换。
- API 新增 `APP_SECRET_KEY_FILE`、`DATABASE_URL_FILE`、`REDIS_URL_FILE`、候选密钥和通知凭据 `*_FILE` 读取；直接值与文件引用同时配置时拒绝启动。
- 新增 `docker-compose.production-secrets.yml`，MySQL 使用镜像原生文件密码，Redis 从 Secret 生成容器临时认证配置，API/Worker/Scheduler 只接收秘密文件路径。
- API 镜像入口以 root 复制只读 Secret 到容器临时文件系统，设置 `app` 用户 `0400` 后通过 `su-exec` 降权，兼容宿主机 root-only `600` 文件。
- 新增 `scripts/manage_production_secrets.py`，提供不可覆盖初始化、权限/完整性检查、Scrypt + Fernet 加密备份、恢复和候选密钥版本轮换。
- 新增本地合同门禁 `pnpm production-secrets:contract` 和 `check.ps1 -IncludeProductionSecrets`。
- 下一步只在当前正式 Staging 证据任务结束后，将目标服务器受控切换到该 Compose 覆盖，并执行首次异地备份恢复及候选密钥轮换演练；不得在运行中的稳定性会话中途更换源版本或密钥。


## 2026-08-11 WP4 第 29 轮执行结果：完整用户等级系统

- 已将用户等级与积分、信誉、产品角色拆分为独立领域；成长值采用不可变事件，等级采用可重建投影。
- 已实现默认五级目录、规则哈希、管理端配置版本治理和发布/回滚后的全量投影重建。
- 已接入每日首次登录、首次验证贡献、有效验证奖励，以及候选奖励失效/恢复的成长补偿。
- 已在服务端执行贡献资格和每日揭示额度，等级升级不授予任何管理权限，也不绕过 N2 角色审批。
- Web/Admin、共享 API 契约和自动化测试已形成闭环。
- 完成度基线更新为核心功能约 98%、本地工程约 99%、综合研发约 90%；生产上线准备度仍约 78%，因为正式 Staging、真实 HA、外部通知和实机验收不因本轮功能代码而自动关闭。
- **下一轮开发入口：** 不再扩展会员、商城或付费等级；优先完成正式 Staging 结论、真实 MySQL/Redis HA 证据、生产 Secret 目标切换、真实通知和实机兼容性验收。
