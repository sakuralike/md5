...
| 点赞、收藏 | 未实现 | 本计划 WP5-I4 |
| 关注、粉丝、公开主页 | 未实现 | 本计划 WP5-I5 |
...
| `/u/:username` | 用户公开主页 | 公开，但遵循用户隐私设置和屏蔽关系 |
| `/settings/community` | 社区隐私与通知设置 | 登录，仅本人 |
...
|---|---|---|---|---:|
| WP5-I3 | 页面与数据读取重构 | 独立首页、帖子详情、发布页、评论游标分页、兼容当前路由 | 进行中：生命周期、游标和独立页面路由已完成 | 4～5 开发日 |
...
> 2026-08-12 第二个开发切片：已新增独立 CommunityHomePage、CommunityPostPage、CommunityPostComposerPage，并接入 /community、/community/posts/:postId、/community/new；原综合页保留为 /community/legacy 兼容入口。详情页已接入评论游标、两级回复和验证后回复。作者编辑/删除、举报及评论治理迁移仍待下一切片收口。
...
- 验收：匿名阅读、验证后发布、锁定拒绝回复、编辑冲突、删除占位、移动端页面测试。

#### WP5-I4：点赞和收藏

- 新增 reaction/bookmark 迁移、服务和投影校验。
- UI 提供可撤销按钮、乐观更新失败回滚和本人收藏页。
- 验收：并发重复点赞只计一次，移除内容禁止新增互动，收藏列表仅本人可见。

#### WP5-I5：公开主页和关系图

- 新增公开资料、隐私偏好、关注、屏蔽、静音。
- 用户主页展示公开帖子、等级和允许公开的关系统计。
- 验收：屏蔽后关注解除、私信/提及拒绝、隐藏列表不旁路泄露。
