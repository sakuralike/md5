...
#### WP5-I7：动态和通知（第 1 个开发切片本地完成）

- 已建立 append-only 动态事件、最新/关注/群组读取模型、游标分页和关注/入组公开偏好；发帖、回复、关注和成员激活动作进入动态事实源。
- 已扩展回复、提及和关注站内通知，保留业务唯一键去重，提供类型筛选、未读数、单条/批量已读及按类型配置的站内/邮件摘要偏好。
- 动态和通知读取复用屏蔽、静音、群组成员资格、内容状态和账号状态过滤；源内容移除后不再返回对应事件。
- 后续切片继续实现点赞摘要聚合、群组申请/决定/角色通知、邮件摘要实际投递、Outbox 重放和未读角标实时增强。
