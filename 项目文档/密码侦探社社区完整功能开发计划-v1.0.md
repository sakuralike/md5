...
| 可配置板块、群组 | COMMUNITY-45～54 本地已实现 | WP5-I6；目标 MySQL 与真实浏览器回归待部署复核 |
| 动态信息流、通知 | WP5-I7 第 1 切片本地实现 | 最新/关注/群组流、回复/提及/关注通知、类型筛选和偏好已落地；聚合点赞、群组治理通知、邮件投递与 Outbox 留待后续 |
| 社区搜索 | 未实现 | 本计划 WP5-I8 |
| 私信、站内聊天 | WP5-I9 异步基础与 WP5-I10 消息举报/批量冷却已完成代码、自动化和 Staging 部署 | 一对一会话、AES-GCM、收件箱、已读、归档/静音、屏蔽、消息举报最小披露、持久化 15 分钟冷却、MySQL 持久事件、Redis 唤醒、独立 SSE 和游标补偿；迁移 `20260912_0065 (head)` 与真实举报/移除/冷却冒烟通过；UAT/Production 未验收 |
...
| `/community/activity` | 最新、关注和群组动态 | 最新流可匿名读取；关注/群组流需登录 |
...
#### WP5-I6：板块和群组（本地代码与自动化完成）

- 已通过 Alembic `20260812_0032` 新增 `community_boards`、`community_groups`、`community_group_memberships`、`community_group_governance_events`，保留 `board_code` 兼容字段并将既有主题回填到板块外键。
- 已完成动态板块目录、板块最小角色与只读/停用状态、管理端板块配置，以及群组发现、群组主页、公开/申请/私密加入模型、邀请、审批、拒绝、移除、退出、角色调整和群主转移。
- 群组主题复用既有主题、评论、点赞和收藏能力；私密群组内容在主题列表、详情、公开主页、提及通知和群组筛选中统一按成员资格过滤，全站治理身份不自动获得普通读取权。
- 高风险板块配置仅管理员且完成 TOTP 后执行；群组治理写入保留幂等键、治理事件和最小化审计。治理人员修改私密群组时只返回最小群组结果，不泄露成员名单。
- Web 已新增 `/community/groups`、`/community/groups/:groupSlug`，发布页支持动态板块和群组目标；Admin 已新增 `/community/settings` 社区板块配置页并显示权限与状态影响。
- 本地专项验收覆盖固定板块兼容、私密内容不可旁路、申请/邀请、群主转移、管理员与版主权限边界及页面渲染；目标 MySQL 迁移和真实浏览器回归仍待部署复核。动态、搜索和通知中心将在 WP5-I7～I8 复用本轮可见性策略。

#### WP5-I7：动态和通知（第 2 个开发切片本地完成）

- 已建立 append-only 动态事件、最新/关注/群组读取模型、游标分页和关注/入组公开偏好；发帖、回复、关注和成员激活动作进入动态事实源。
- 已扩展回复、提及、关注、点赞摘要和群组站内通知，保留业务唯一键去重，提供类型筛选、未读数、单条/批量已读及按类型配置的站内/邮件摘要偏好。
- 已新增按用户 UTC 固定窗口的持久化邮件摘要批次与关联项（Alembic `20260817_0047`）：站内与邮件通道独立排队；邮件 Worker 每两分钟领取成熟批次，复核偏好、屏蔽、内容状态和群组可见性后，通过通知网关投递最小披露的纯文本摘要。稳定 Message-ID、有限退避重试、Admin 聚合状态和用户删除级联清理均已实现。
- 站内列表、未读数、单条/批量已读与 SSE ready/event 均按 `in_app_enabled` 过滤，防止仅启用邮件的通知事实泄露到站内通道。
- 本地自动化、SQLite 迁移往返与前端统一门禁已完成；真实 Staging SMTP 接收、目标 MySQL/Redis 运行时与容量验证待本轮部署记录。
...
## 21. 2026-08-12 WP5-I7 第 1 个开发切片状态

- 新增 `community_activity_events`、`community_activity_preferences`、`community_notification_preferences` 与 Alembic `20260812_0033`；动态事件以来源类型和来源 ID 唯一约束保证同一业务事实不重复落库。
- 新增 `GET /community/activity` 及动态偏好 API，支持 `latest`、`following`、`groups` 三类游标流；匿名用户仅可读取公开最新流，私密群组内容只对活跃成员可见。
- 发帖、回复、关注、公开群组直接加入以及审批/邀请成为活跃成员均写入动态事实；关注和入组公开偏好只影响未来事件。
- 通知中心新增回复、关注类型、类型过滤和按类型的站内/邮件摘要偏好；站内事件继续使用最小化纯文本摘要、本人资源边界、业务唯一键和屏蔽关系过滤。
- Web 新增 `/community/activity` 页面和主导航入口；通知中心增加多类型筛选、接收偏好、可空内容引用和用户主页回退链接。
- COMMUNITY-55～61 已完成本地代码和自动化；COMMUNITY-69～73、76 已完成回复/提及/关注子集。点赞摘要、群组治理、私信/治理结果通知、邮件摘要实际投递、Outbox 与 SSE 仍属于 WP5-I7 后续切片。

## 22. 2026-08-16 WP5-I9 浏览器验收切片状态

- Web 已完成 `/community/messages` 收件箱、`/community/messages/:conversationId` 会话页及从公开资料发起私信的主旅程。
- Chromium、Firefox、WebKit 使用发送者、接收者和非成员三个合成身份完成 3/3 定向验收；非成员访问会话的 HTTP 404 是明确断言的权限边界。
- 通知 SSE 的 CORS 预检允许 `Last-Event-ID`；Web E2E 使用生产形态的同源 `/api/v1` 代理，独立源合同由 API 回归测试覆盖。
- Admin 通知 Outbox 已能显示并筛选 `direct_message`；测试板块固件不再写入无效的 `guest` 角色。
- `pwsh ./scripts/check.ps1 -SkipInstall -IncludeE2E` 已通过，其中 Chromium Web/Admin Playwright 42/42、Windows Desktop 29/29。
- WP5-I9 本地代码与自动化闭环完成；本地提交 `f7da09b` 的 Git Tree `265bfdb` 已通过 Git Data API 同步为远端等价树提交 `4c3376e`，并部署到 Staging。Staging 独立复验确认迁移 `20260816_0044 (head)`、2 个 API、3 个 Worker、API/Worker/Scheduler 与 Admin 候选镜像、Web 保持既有镜像、关键 HTTP 状态、`Last-Event-ID` CORS 预检、私信密钥隔离和长事务为 0；WP5-I10 实时 Pub/Sub、消息举报和治理能力仍未完成，UAT/Production 未验收。


## 23. 2026-08-16 WP5-I10 私信实时事件交付状态

- COMMUNITY-85 的三类领域事件 `message.created`、`conversation.read`、`unread.changed` 已实现；事件按用户连续序号持久化，不保存私信正文、密文、nonce、密钥版本或令牌。
- Alembic `20260816_0045` 新增用户流位置和持久事件结构；MySQL 8 为目标事实源，本地 SQLite 仅用于自动化。隔离临时库已完成 `upgrade 0045 -> downgrade 0044 -> upgrade 0045`，最终为 `20260816_0045 (head)`。
- `GET /api/v1/community/direct-messages/stream` 使用 function-scope 认证、`Last-Event-ID` 补偿、异常游标重置和数据库短轮询降级；Redis Pub/Sub 只发布用户最大事件序号用于跨实例唤醒。
- Web 已接入独立私信流客户端、Store 和用户级 `sessionStorage` 游标；收件箱、账户菜单、会话消息与逐消息已读回执实时同步，并避免普通未读/已读事件触发重复消息拉取和冗余已读 PATCH。
- 本地证据：五组定向 API/迁移/加密测试通过；Web 45 个测试文件、78 项测试通过；Chromium、Firefox、WebKit 双用户实时与断线补偿旅程 3/3 通过；`pwsh ./scripts/check.ps1 -SkipInstall` 完整统一门禁通过。
- 远端交付：本地代码提交 `c658cff7aade` 的 Git Tree `72337848988a` 已同步为分支 `codex/wp5-i10-direct-message-realtime` 的远端等价树提交 `1a59275d042d`，`equivalent_tree=true`。
- Staging 交付：修订 `1a59275d042d` 使用备份 `/opt/password-detective-backups/20260816T173933Z-1a59275d042d` 完成 MySQL 迁移 `20260816_0045 (head)`；API 2、Worker 3、Scheduler、Web 与保留的 Admin 均健康。未登录私信流 401、CORS 预检 200、授权 `ready`、API-2 到 API-1 的 Redis 唤醒 0.108 秒、Redis 停止后的数据库补偿 0.005 秒、恢复后 Redis 健康、事件无敏感字段且长事务为 0。
- 公网 Web `:5173`、消息页与 Admin `:5174` 返回 200；API `:8000` 仅供服务器内网/代理使用，公网未开放。Staging 复验另发现与 WP5-I10 无关的既有隐私设置缺陷：`PATCH /community/me/privacy` 写搜索 Outbox 时 `document_version` 超出 MySQL 列范围并返回 500，后续轮次须修复。Staging 不代表 UAT 或 Production 批准。


## 24. 2026-08-16 WP5-I10 搜索 Outbox 文档版本溢出修复状态

- 已修复 Staging 复验发现的独立隐私设置缺陷：资料 `updated_at` 微秒版本写入 32 位搜索 Outbox 列造成 MySQL 1264。
- Alembic `20260816_0046` 将 `community_search_outbox` 与 `community_search_documents` 的 `document_version` 同步扩宽为 `BIGINT`；不改变版本生成、Outbox 去重或搜索投影的较新版本优先语义。
- 本地完整 API、SQLite 迁移往返和前端质量门禁通过；`check.ps1` 未因代码失败而中断，而是当前 worktree 虚拟环境布局及 `cryptography` DLL 本机加载问题，已以等价子门禁覆盖。
- 远端等价树提交 `82fef8257ab7` 已部署 Staging：迁移 `20260816_0046 (head)`、双列 `bigint`、真实隐私设置 200、Outbox 版本 `1786914188170251`、API 2/Worker 3/Scheduler 健康、长事务 0。
- 该缺陷不再阻断 Staging，但 UAT/Production 仍未验收或批准。
