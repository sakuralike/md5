...
| `announcements:read` | 读取桌面端公告 |
| `updates:read` | 检查桌面版本和下载信息 |
...
挑战绑定用户、应用、安装实例、nonce、指纹、版本、Scope 和过期时间。第三方回执使用独立协议版本 `third-party-desktop-receipt-v1`，签名覆盖应用、用户、安装实例、挑战、nonce、档案指纹、候选摘要、结果、压缩格式、版本和验证时间。
