...
## 15. 2026-08-16 本地实施回填

- 已按方案 B 落地 Alembic `20260816_0045`、用户流位置、持久事件、提交后 Redis 唤醒、独立 SSE 和 Web 独立 Store；实现边界未改为普通通知事件或仅内存广播。
- 实测确认幂等完成记录在 `_mutate_with_idempotency` 内提交消息与事件后，路由才查询最新用户流位置并发布 Redis 唤醒；Redis 异常被隔离，不回滚已提交业务事务。
- Web 会话页只监听 `message.created` 对应的消息刷新版本；`conversation.read` 和 `unread.changed` 仍更新会话/收件箱状态，但不再重复拉取消息。服务端列表快照已确认的 `last_read_sequence` 不再触发冗余已读 PATCH。
- 浏览器测试对刻意离线和页面重载产生的网络取消按明确转换窗口隔离，窗口外仍严格断言发送方/接收方请求失败、控制台错误和页面错误为空。
- 本地验证已完成：五组定向 API/迁移/加密测试通过；Web 45 文件 78 测试通过；三浏览器双用户旅程 3/3；隔离迁移往返最终 `20260816_0045 (head)`。
- `pwsh ./scripts/check.ps1 -SkipInstall` 完整统一门禁已通过；远端 ref/tree/commit 和 Staging 多实例证据仍按交付步骤继续执行，在这些证据完成前不代表 UAT 或 Production 批准。
